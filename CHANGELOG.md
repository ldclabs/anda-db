...
- **BM25 flush callback signatures** — `BM25Index::flush` / `flush_with` / `store_dirty_buckets` callbacks are now plain `FnMut`/`FnOnce` closures returning a future and receive owned `Vec<u8>` blobs (previously `AsyncFn*` over `&[u8]`); the `AsyncFn*`-over-borrow pattern made every downstream `tokio::spawn` of a flush fail to prove `Send` (rustc "implementation of `Send` is not general enough").

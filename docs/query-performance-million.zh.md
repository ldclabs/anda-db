...
`4afaef1` 把 SEARCH 缓存改按 Space 序号计键，并简化了分页与游标代码。它与 `83d6427` 用同一 harness、Cargo.lock、编译器和构建参数做了配对复测，[完整数据](benchmarks/anda_query_scale_2026-09-26-rerun/README.md)单独存放。

| 1,000,001 行，热查询 P50 | 83d6427 | 4afaef1 |
| --- | ---: | ---: |
| 稀有键与稠密键交集，10 个结果 | 0.001230 ms | 0.001250 ms |
| 稠密首屏，50 个 ID | 0.002292 ms | 0.002292 ms |
| 稠密深页，50 个 ID | 0.002437 ms | 0.002417 ms |
| 完整 ID 集 | 30.864 ms | 30.274 ms |

10 万和 100 万行下，各项差异都在 ±4% 以内，没有超出同一版本两轮之间的波动。两个版本新增的堆分配字节完全相同。百万行 Nexus 探针在两个版本下都通过全部断言，每个用例的 Concept GET 数也一致。两个 harness 都不经过 SEARCH 缓存，这部分改动由回归测试覆盖，复测不能说明它的性能。

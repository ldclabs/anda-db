...
基于 `5a093df` 实施，全程未使用 subagents，未增加依赖；桶与元数据的 CBOR 布局不变。

- [x] 不再统计查询次数。原实现每次查询都对同一个原子量自增，并发读者会争抢同一条缓存行。`BTreeStats.query_count` 字段保留（旧版本解码元数据时需要它），其值停留在加载时的数值。
- [x] 加载结束后由现存倒排表一次性构建有序键集合，不再按哈希顺序逐桶插入。加载因错误中止时同样会构建，已加载的倒排表仍可被范围查询访问。
- [x] 不含倒排表的桶不再进入清单：不再被写成空对象、也不会在每次打开时被读取；其原有对象（包括旧版本提交的空对象）作为废弃对象返回。
- [x] 删除无任何读者的倒排表更新计数（每个键省 8 字节）。持久化三元组在该位置写 `0`，读取时忽略。
- [x] 新增 `BTreeKey` 约束别名与统一的错误构造函数；删除 `load_metadata` 中只会被 `load_buckets` 丢弃的占位桶。

### 测量结果

Apple M1 Pro（10 核），rustc 1.98.1，release 配置（`opt-level=z`，LTO）。用一个临时 A/B 测试程序（未提交）分别针对两个版本构建，交替各跑 3 轮，表中为中位数。

| 场景 | 修改前 | 修改后 |
|---|---:|---:|
| 10 万键点查，1 / 2 / 4 / 8 线程（合计 M ops/s） | 30.2 / 25.5 / 24.9 / 19.7 | 31.6 / 52.3 / 86.5 / 76.6 |
| 加载 100 万 `u64` 键（每轮 5 次取最优） | 492 ms | 359 ms |
| 加载 100 万 `String` 键（每轮 5 次取最优） | 1,068 ms | 772 ms |
| 4 KiB 桶中 2 万个键，删除最旧的 1.9 万个后 flush：清单条目 / 写出对象 / 空对象 | 67 / 64 / 63 | 4 / 1 / 0 |

## 2026-09-23 后续维护

基于 `eb950fd623796b47ba56f6ba5db5120117d9cf4f` 实施，全程未使用 subagents，未增加依赖，未改变公开 API 或持久化格式。

- [x] 在同一次 DashMap 分片锁内原子删除最后一个 ID 及其倒排表条目。单条和批量删除统一使用 `remove_if_mut`；批量删除直接累计桶记账增量，省去中间删除结果向量。受控线程交错测试覆盖临时空倒排表、同 ID 重新插入、唯一性约束及 flush/reload。
- [x] 压缩大小估算不再依赖旧桶编号的编码宽度。固定按 5 字节估算桶编号，使重复装箱稳定。回归覆盖 32 和 300 个桶、无变化 flush 和重新加载。
- [x] 移除范围与前缀查询中的全分片空检查；空索引仍先执行查询合法性验证。
- [x] 大量删除后才按比例收缩倒排向量和位置表，并为再次增长预留空间。10 万 ID 的回归测试覆盖两个容器的容量、剩余成员、重新增长及非尾部删除。
- [x] Include 在已有向量中排序去重；查询转换直接收集装箱后的节点；准备好的清单直接移入提交状态，省去额外克隆。
- [x] 基准补充 Eq/Ge 与直接点查对照、不同 Include 大小、删除后剩余堆分配以及重复压缩与 flush。

### 实测结果

环境为 arm64 macOS、10 个可用 CPU、rustc 1.98.1、工作区 bench 配置（`opt-level=z`、LTO）。前后使用相同的计数分配器和扩展后的基准程序；基线采用修复前实现。纯 CPU 测量期间无并行编译，查询取 100 个样本，压缩/flush 取 20 个样本。Eq/Ge 每个计时样本执行 1,000 次查询，首个命中即停且不分配结果；表中已换算为单次查询时间。

| 场景 | 修复前 | 修复后 |
|---|---:|---:|
| 直接点查，中位数 ns/次 | 29.9 | 29.3 |
| RangeQuery::Eq，中位数 ns/次 | 372.1 | 34.0 |
| Ge 首个命中，中位数 ns/次 | 454.8 | 114.0 |
| Include 128 个不同键，中位数 μs/次 | 3.833 | 1.583 |
| Include 128 分配次数/次（含输入和结果） | 14 | 2 |
| Include 4,096 分配次数/次（含输入和结果） | 378 | 2 |
| 10 万 ID 删至 5 个后的剩余堆分配 | 3,277,224 B | 3,608 B |
| 10 万 ID 删至 4 个后的剩余堆分配 | 1,048,960 B | 1,392 B |
| 32 个桶，无变更连续压缩 20 次的桶写入数 | 640 | 0 |
| 300 个桶，无变更连续压缩 20 次的桶写入数 | 6,000 | 0 |

剩余堆分配以已创建的空索引为基线，不是进程 RSS。剩余 5 个 ID 时向量容量从 131,072 降为 126。本次单次内存回收测量中，删除 99,995 个 ID 的时间从 11.29 ms 变为 11.70 ms；缩容用偶尔的重新分配/重建哈希表换取受控的剩余容量。现有 10 万 ID 未命中删除和尾部删除中位数仍为 42/125 ns。清单调整减少了 flush 分配次数，不宣称降低了 flush 延迟。这些微基准不能直接推导业务整体吞吐。

原始结果：[修复前](../rs/anda_db_btree/benches/results/review-2026-09-before.json)、[修复后](../rs/anda_db_btree/benches/results/review-2026-09-after.json)。

### 验证

工作区全部 1,734 项测试通过，1 项夹具生成测试按设计忽略，其中包括 B-tree 恢复、崩溃恢复与历史格式兼容测试。B-tree 全目标测试（含原子文件写入示例）也通过。工作区全特性 check、全目标/全特性 Clippy（`-D warnings`）、格式、agent 文档一致性及 32 个本地文档链接检查均通过。新增正确性和容量回收回归均先在修复前实现上失败，再在修复后通过。

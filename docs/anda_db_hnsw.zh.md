...
Cosine 查询范数只预先计算一次；不可变节点的范数会被缓存，并在查询、选邻、剪枝和删除重连时复用。API 始终返回欧氏距离本身，而非平方 L2。
...
启发式选邻会将相同向量的候选推迟到最后回填，优先为不同方向保留名额。剪枝与删除重连使用 f32 重新评分候选，避免将量化后的 bf16 边权与完整精度的节点间距离直接比较。这可以防止重复向量簇剪掉新插入的不同向量的全部入边，但不保证任意数据或删除顺序下的全局连通性。节点和边的持久化格式保持不变。

串行写入者复用有容量上限的构建工作区，包括搜索缓冲区、候选选择缓冲区和节点间距离缓存。正常的现代快照加载会跳过旧格式距离修复所需的查找表，也不再为每层邻接表分配去重集合。
...
上传并发度支持 1–64。在途的节点回调缓冲区总和及后续 IDs/元数据回调缓冲区严格受配置的字节预算约束。单个超出预算的载荷将直接返回错误。所有节点、IDs 和元数据的载荷大小都在调用任何写入回调前检查；IDs 与元数据在此预检阶段编码，并保留至各自的回调执行。字节预算约束回调载荷，不是整个快照及暂存缓冲区的总内存上限。使用 `cbor2::serialized_size` 计算精确的节点大小。IDs 对象在所有节点回调成功后才写入；元数据最后写入。节点回调返回 `false` 会停止后续写入且不提交。在发生错误或终止时，flush 启动的所有节点回调在方法返回前均会被等待完成。取消操作可能导致底层已发生部分物理写入，因此脏数据证据会被保留以便后续恢复。
...
回归测试套件覆盖数值溢出、ID 复用、缓冲 writer 写入失败、停止/取消契约、墓碑代际、混合镜像恢复、事务化加载重试、查询边界以及有界并行上传。召回率测试采用确定性数据与随机种子，覆盖四种度量的均匀数据及带正负值的聚类数据，并用绝对值容差处理负距离的边界并列。重复向量回归测试还会检查不同向量插入后的入边，以及持久化前后的自查询结果。
...
[2026 年 9 月对比](../rs/anda_db_hnsw/benches/README.md#september-23-2026-comparison)记录了每组 1,000 条向量、三轮测试的中位数：构建耗时下降 6–36%，加载分配次数减少约 29%，768 维 Cosine 的删除重连耗时下降约 42%。欧氏查询耗时增加 5–8%，召回率保持或提高。工作区复用会增加驻留堆内存，该取舍也已在基准说明中记录。用于应用容量规划前，请先核对原始数据与测试环境。

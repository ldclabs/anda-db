...
- **倒排项 `(bucket_id, Vec<(doc_id, tf)>)`**：`bucket_id` 标识当前持有该词项的分桶；列表为每次 `insert` 维护一条记录。只有在使用非原始文本执行 `remove` 且重新插入相同 ID 时，单个文档才可能在列表中出现多次——评分阶段按 `doc_id` 聚合，重复项只计算一次；`remove` 在单遍扫描中清理该 ID 的所有条目；重新加载时会自动剔除已不存在的废弃记录。对于存活文档的重复条目系统予以保留，避免在 `insert` 热路径上引入与倒排表长度成线性的扫描。调用方若无原始文本，应使用不需要文本的 `purge_ids`。
...
`is_dirty()` 当且仅当 `dirty_version > saved_version` 时成立，这赋予了 `flush` **幂等且线性增量**的特性：成功写入的分桶仅记录写入时观察到的 `dirty_version`。若在写入期间并发发生新修改，该分桶仍保持 dirty，并在下一次 flush 时继续被写出。
...
- 插入、移除、清除 (purge) 与搜索支持多线程并发调用。针对同一文档 ID 的写操作通过内部条带锁进行串行化，条带跨越文档成员资格、倒排表与分桶记账。批量 purge 按照文档 ID 升序依次获取条带锁。
...
- `flush` 捕获脏版本及下一次清单，然后逐一序列化并上传脏分桶。仅属于该分桶的倒排表会被写出；文档长度直接从这些倒排表中推导得出。
...
优先级顺序为：`OR < AND < NOT`。操作符大小写敏感且两端必须带有空格（`NOT` 紧跟空格）：`a and b` 被视为三个普通单词，而 `NOT NOT a` 构成两层双重否定。核心特性：

- **多词项查询默认为 OR**：在 `search` 与 `search_advanced` 中，`"quick fox"` 与 `"quick OR fox"` 返回完全相同的结果。
...
- **候选集执行优化**：AND 优先求值高选择性操作数，保留全局 DF/IDF 参与评分，并将后续计算严格限制在幸存候选集内。NOT 过滤器只进行集合成员判定，不浪费算力计算被丢弃项的 BM25 分数。IDF 在单次查询内缓存，并使用 `ln_1p` 确保数值稳定性。
- **`NOT` 补集防线**：所有物化的补集（包括相对候选集的补集）均严格受 10,000 篇文档上限约束。简单的 `AND NOT term` 与 `AND NOT (a OR b)` 过滤器直接从对应倒排表中剔除 ID，无需复制候选集。前导的成对 NOT 会被提前抵消，例如 `hello AND NOT (NOT world)` 会被作为正面过滤器求值，从而在更大结果集上也能正常执行。真正需要全量候选空间且超出上限的混合极性表达式将返回错误。`try_search_advanced` 会暴露此类资源错误；`search_advanced` 遇到错误则返回空结果。严格解析会拒绝括号/NOT 嵌套预算耗尽的输入，而非静默曲解查询意图。
...
1. **移除操作需要原始文本**：`remove(id, text, now_ms)` 依赖对原始文本重新分词以定位倒排表。传入错误文本虽能隐藏缺失文档，但会导致陈旧倒排条目残留；复用该 ID 可能会暴露历史词项。务必传入原文本，或在复用前执行 purge。缺失文档的陈旧条目会在下一次加载索引 (`load_buckets`) 时被清理，而不会被只按词项重排的 `compact_buckets()` 清理。若文档确实无法找回（例如文档内容已丢失的灾难恢复场景），应改用 `purge_ids(&BTreeSet<u64>, now_ms)`：它针对传入的 ID 集合单遍扫描所有倒排表，从 `doc_tokens` 与 `total_tokens` 中扣除对应 ID，并将受影响的分桶标记为 dirty。这是维护级别的 $O(\text{索引大小})$ 操作，应一次性传入所有废弃 ID，切忌在循环中逐个调用。

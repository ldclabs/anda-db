...
`DEFINE PREDICATE` / `DEFINE CONCEPT TYPE` 在 `propose_schema` 权限下向本 Space 的草稿词汇 `kip://local/draft@0.0.0`（§20.16）添加符号：每次都是独立的治理事务，草稿包由 Schema Lock 合成，之后的激活都会保留它；`Session::promote_draft_symbol`（`manage_schema`）把草稿符号映射到已安装包的同类符号，两者按同一血缘读取——匹配与身份都是如此，在草稿下写入的元组、逻辑键或 `CLIENT KEY` 之后仍解析到同一个元素。`Session::repair_recording` 修复记录者抽取错的主张（§57.8）：核对来源摘要与定位器，并对错误断言做版本 CAS，然后追加一个终态的 `recording_repair` Activity，在断言受保护的治理块里标记失效（载荷与生命周期不变，`_system.recording_validity` 显示修复记录），投影不再采纳它。`Session::record_exposures` 在认知存储之外维护 §66.8 曝光日志，`read_exposures` 需要 `read_audit`。引擎仍只声明 `KIP-Core`、不声明 `KIP-CognitiveMemory`：GradingState 与血缘字段只拒写、读时不计算，选择依赖（§57.7）也未评估。`CognitiveNexus::set_host_capabilities` 用于 Brain 声明它提供的 Memory Interface 与 Brain Runtime 能力。

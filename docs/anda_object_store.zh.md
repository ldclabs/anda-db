...
常规的 put 操作先写入新的 generation payload，发布对应的元数据，随后以尽力而为的方式移除被替换的旧 payload。每个 generation 路径都是全新的，因此 payload 的 put、复制与分块上传都以覆盖模式单次写入：不做存在性探测、不用条件创建、也不做碰撞重试。下文的 128 位随机盐加进程级序号使碰撞概率可以忽略，单写入进程契约则排除了同一 key 上的外部写入者。

分块上传 (Multipart) 会先分配并注册全新 ID，随后直接启动后端的上传流程。它不会在磁盘持久化零字节的占位对象，因此中止操作 (abort) 或初始化失败绝不会泄露占位文件或产生多余的对象版本。在所有情况下，元数据始终是逻辑层面的唯一提交点。
...
在元数据发布前发生的失败，会保留之前已提交的对象处于可读状态。在发布后发生的失败，可能意味着即使调用方收到了错误，新写入实际上已在后端生效。当元数据 put 或 delete 已到达后端却返回错误时，该 key 会从共享元数据缓存中驱逐，下一次查找将从存储后端重新加载已提交的权威状态。若 future 被取消，同步的 `Drop` 保护守卫无法执行异步的单 key 驱逐，只能使整个缓存失效。常规成功操作仅更新其自身的 key。

读取器先解析提交点，然后读取对应的不可变 payload。若该 payload 已被替换并回收，`get`、范围读取与复制操作在遇到 `NotFound` 时会刷新元数据并重试一次；期间已被删除的 key 会被驱逐并报告 `NotFound`，其他刷新失败不改动缓存。这并非通用网络重试策略。由于历史 payload 会被回收，系统不支持历史版本寻址。

HEAD 请求（带 `head` 且不带 range 的 `get_opts`）在逻辑时间戳已知时仅由提交点应答，不再请求 payload。因此它不能证明 payload 仍然存在，也不返回后端 attributes；与列举一致，它报告的是逻辑对象，而逻辑对象存在当且仅当其提交点存在。0.10 之前的旧文档仍会访问旧版 payload，并报告其后端时间戳。

## 并发控制与条件操作

同一 wrapper 包装器的克隆实例共享其元数据缓存、按 key 的计算锁以及在途的 generation 注册表。后端自身无需实现 `Clone`。单独构建的不同包装器实例即使位于同一进程内，也不会自动共享该协调机制。

每个逻辑存储空间应保证单一协调写进程。在共享的包装器实例内部，针对同一 key 的数据变更被串行化，且该 key 的每次提交、重新加载与驱逐都发生在其按 key 的临界区内。因此缓存中的提交点就是该实例已提交的权威状态：put、复制与删除在缓存持有该 key 时直接从缓存解析当前文档，仅在未命中或条目不再通过校验时才读取后端。在跨独立实例的场景下，针对不存在提交点的 `PutMode::Create` 由后端的元数据条件写入负责仲裁。跨实例的 `Overwrite`、`Update` 与 GC 必须依赖外部的单写入者协调机制。

包装器不会在软件层面模拟后端的条件写入。因此，若存储后端自身不支持条件元数据 put，则无法提供跨实例的逻辑 `Create`；在此类后端上必须依赖外部协调机制。

- `Create` 绝不替换已有提交点，即使元数据无法解码或校验失败。错误的加密密钥或调整后的资源限制不能让已有对象被视为不存在。显式 `Overwrite` 可以重建异常元数据；清理时绝不会跟随不可信指针。
- `Update` 要求提供当前逻辑 ETag，并在按 key 锁定的临界区内与已提交文档比对。元数据无效或不存在均无法满足该条件。不支持 `UpdateVersion::version` 并会返回错误。
...
条件 copy/rename 在确认源元数据后、复制 payload 前检查目标提交点。目标已存在时直接失败，不会产生多余的 generation。最终的元数据条件写入仍会仲裁检查之后才创建的目标。
...
`get_opts(range)` 会将请求范围对齐扩展至完整的加密块边界，并在校验通过后对明文进行裁剪。对齐后不超过 1 MiB 的范围以一次 body 读取取回并原地解密（后端交出独占缓冲区时无需拷贝，本地文件即是如此）；否则文件后端会按 8 KiB 分块流式读取，且每块都要在阻塞线程池上读一次。更大的范围走解密流，从上游缓冲区中按有界批次（最多 64 KiB 或单个加密块，取较大者）复制到预分配的批次缓冲区。裁剪后的结果小于其明文缓冲区一半时，会复制到独立的小缓冲区，因此持有一字节范围不会保留整个加密块；完整流式批次仍直接移交缓冲区。上游存储后端可能仍保留其自身的缓冲区。HEAD 请求与空对象读取不会触发解密。`get_ranges` 同样会在响应缓冲区独占时原地解密。
...
提交时间戳在按 key 互斥锁内、通过重试检查后、真正发布新元数据之前生成，加密元数据也在此时签封。发布失败后的重试使用新时间戳；如果 generation 已提交但响应丢失，重试保留已提交的时间戳和 ETag。
...
- `with_meta_cache_bytes(bytes)` 配置内置加权缓存的字节预算。
...
Builder 将内置缓存的构造推迟到 `build()`，连续设置配置不会反复分配和丢弃缓存。

冷读取和 listing 都在与提交操作相同的按 key 互斥锁内加载、校验并填充缓存。重复 listing 及随后的读取可以复用这些条目，避免重新获取 sidecar；listing 不会在新提交之后插入旧快照。已删除条目和兼容模式下无法解码的 CBOR 会被跳过且不缓存，语义或认证失败仍会返回错误。
...
测试套件涵盖 InMemory/LocalFileSystem trait 一致性、旧版格式兼容、CAS/ABA 与并发检查、认证清理、未知结果与取消处理、分块上传重试、缓存信任边界、大小配额、范围查询 oracle 及请求/响应 Extensions。基准测试套件记录延迟、分配次数、累计分配字节数与最大单次分配。微小范围测试另行记录持有结果期间的存活堆分配，条件复制测试记录后端复制次数和剩余目标 generation 数量。真实的云端集成与物理断电测试需结合具体生产部署环境单独验证。

### 本轮修复的性能对照（2026-09-23）

使用相同的扩展 benchmark 和解析后的依赖，对比 `308c5cd` 的实现与本轮修改。环境为 macOS arm64、Rust 1.98.1，两侧均设置 `CARGO_PROFILE_BENCH_LTO=false CARGO_PROFILE_BENCH_OPT_LEVEL=3`。交替运行修改前后版本各三次，每个场景预热两次、采样 15 次；下表耗时为三次运行中位数的中位数。这是带分配计数器的本机微基准，不代表云端吞吐。

| 场景 | 修改前 | 修改后 |
| --- | ---: | ---: |
| 默认 256 KiB 块、偏移 100 的一字节范围：持有堆字节数 | 262,168 | 1 |
| 同一范围的读取耗时（µs） | 119.38 | 119.04 |
| 目标已存在的条件复制耗时（µs） | 4.46 | 0.96 |
| 条件复制共尝试 17 次的后端复制次数 | 17 | 0 |
| 100 个加密对象，冷列举后的重复列举（µs） | 180.42 | 61.29 |
| 100 个加密对象，冷缓存列举（µs） | 179.38 | 297.42 |
| 100 个已缓存普通对象的列举（µs） | 52.92 | 58.04 |
| 加密对象全量读取 4 MiB（µs） | 2,341.92 | 2,371.33 |

小范围读取减少了结果长期持有的内存，仍需解密完整块。列举填充缓存增加了首次扫描成本，后续扫描可节省 sidecar 读取及认证。请求计数回归测试确认：三个对象的两次列举加一次 head，在缓存启用时仅获取三次元数据，禁用缓存时则为七次。条件复制被拒绝后只保留原目标 generation；成功的条件复制会多一次元数据 HEAD，以避免目标已存在时的无效 payload 复制。

### 本轮修复的性能对照（2026-09-24）

对比 `46e3798` 与本轮修改。环境为 macOS arm64、Rust 1.98.1，两侧均设置 `CARGO_PROFILE_BENCH_LTO=false CARGO_PROFILE_BENCH_OPT_LEVEL=3`，使用相同的扩展 benchmark（本地盘加密用例为本轮新增）。修改前后交替运行各两次，下表耗时为两次运行中位数的中位数。微秒级的 InMemory 用例在两次运行间波动可达 2 倍，未观察到超出噪声的差异。

| 场景 | 修改前 | 修改后 |
| --- | ---: | ---: |
| 本地盘加密读取 64 KiB（µs） | 263.17 | 126.50 |
| 本地盘加密读取 1 MiB（µs） | 1,865.71 | 718.48 |
| 同一 1 MiB 读取：累计分配字节数 | 4,200,891 | 1,055,158 |
| 本地盘加密 `get_ranges`，64 KiB 对象（µs） | 152.23 | 106.71 |
| 本地盘加密 HEAD，已缓存（µs） | 56.23 | 0.88 |
| 本地盘加密覆盖写已缓存 key（µs） | 828.29 | 483.54 |
| MetaStore 本地写入，关闭 fsync（µs） | 763.17 | 397.98 |
| MetaStore 本地写入，开启 fsync（µs） | 33,091.96 | 19,441.31 |

不超过 1 MiB 的范围以一次 body 读取代替 8 KiB 分块，并直接在文件后端返回的缓冲区中解密。已缓存 key 的提交省去元数据 GET，payload 写入不再使用条件创建，HEAD 不再请求 payload。

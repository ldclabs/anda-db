...
`get_opts(range)` 会将请求范围对齐扩展至完整的加密块边界，并在校验通过后对明文进行裁剪。解密流从上游缓冲区中按有界批次（最多 64 KiB 或单个加密块，取较大者）复制数据。裁剪后的结果小于其明文缓冲区一半时，会复制到独立的小缓冲区，因此持有一字节范围不会保留整个加密块；完整流式批次仍直接移交缓冲区。上游存储后端可能仍保留其自身的缓冲区。HEAD 请求与空对象读取不会触发解密。

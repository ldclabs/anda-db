...
- [x] **2. 旧版字段编号复用。** 历史不明时拒绝直接分配；`SchemaHistoryRecovery` 从完整原始数据恢复水位，集合升级自动执行扫描，包含未登记文档以及 mutation intent 的 previous/proposed 图像。
...
- [x] **6. Json 校验。** 拒绝 Bytes、非文本键及其嵌套形式；必填 Json 区分字段缺失与显式 JSON null。
...
- [x] **14. 规范值快速路径。** Scalar、Text、Bytes、Vector 直接保留数据；规范 Vector 的 `coerce` 不再构建 CBOR 中间数组。

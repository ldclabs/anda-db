/**
 * Bundled Schema Package artifacts — GENERATED FILE, DO NOT EDIT.
 *
 * Source of truth: `rs/anda_cognitive_nexus/profiles/*.json`, which are
 * themselves vendored from the specification repository. Regenerate with
 * `pnpm run codegen:profiles`.
 *
 * Installing is not activating (§20.12). Bundling the bytes says nothing about
 * which Space may resolve symbols through them.
 */

import type { SchemaPackage } from './package.js'

/**
 * KIP Cognitive Memory Profile, version 2.0.0.
 *
 * Vendored verbatim from `rs/anda_cognitive_nexus/profiles/cognitive-memory-2.0.0.json`.
 */
export const COGNITIVE_MEMORY: SchemaPackage = {
  "format": "KIP-Schema-Package",
  "format_version": "2.0-draft",
  "manifest": {
    "package_id": "kip://profiles/cognitive-memory",
    "version": "2.0.0",
    "package_ref": "kip://profiles/cognitive-memory@2.0.0",
    "name": "KIP Cognitive Memory Profile",
    "description": "Standard portable memory ontology for KIP 2.0 Brains.",
    "publisher": "urn:kip:publisher:ldclabs",
    "purpose": "standard_profile",
    "stability": "normative_draft",
    "executable": false,
    "source_document": "profiles/CognitiveMemoryProfile-2.0.md",
    "validation_schemas": [
      {
        "id": "urn:kip:2.0:schema:projection",
        "content_digest": "sha256:a4faae39c8cb0d41b28ed84e13c37d6cf604b054dde94746ccbe61a01ccca13f"
      },
      {
        "id": "urn:kip:2.0:schema:cognitive-records",
        "content_digest": "sha256:82d2ce4801c1e689a31836375a731e780e1d6c4d8ca52925956f674c6d3ca4d1"
      },
      {
        "id": "urn:kip:2.0:schema:element",
        "content_digest": "sha256:07b6f9cfff83134e72424c34a7c580ccac6f9cc9667301cadfd736d5e510824f"
      },
      {
        "id": "urn:kip:2.0:schema:capsule",
        "content_digest": "sha256:474d03b74ed04fbaa7b513510bdc84c07807128bea3664f94f4fe0ea502217d3"
      },
      {
        "id": "urn:kip:2.0:schema:schema-package",
        "content_digest": "sha256:226e45427d6498bdeab706999688fbd13a7f2dc35fdc0c7873f4dbb9e2920fcd"
      },
      {
        "id": "urn:kip:2.0:schema:common",
        "content_digest": "sha256:74a98cb23df9d6d4ebee5b95cab53b09f15f6a55a854be408005a1113e68a7c1"
      },
      {
        "id": "urn:kip:2.0:schema:change-envelope",
        "content_digest": "sha256:6ee11019dc033e176242d244dd49282e6e0aa83edb9046e535a5cd668849dd14"
      }
    ]
  },
  "dependencies": [
    {
      "package_id": "kip://core",
      "version": "2.0.0",
      "package_ref": "kip://core@2.0.0",
      "required": true
    }
  ],
  "definitions": {
    "concept_types": {
      "Person": {
        "ref": "kip://profiles/cognitive-memory@2.0.0/Person",
        "kind": "ConceptType",
        "description": "Semantic person/actor used in cognitive content; never an authenticated Principal or ActorBinding.",
        "attributes": {
          "open": true,
          "fields": {
            "display_name": {
              "type": "string",
              "required": false,
              "mutable": true
            },
            "aliases": {
              "type": "array",
              "required": false,
              "mutable": true
            },
            "description": {
              "type": "string",
              "required": false,
              "mutable": true
            }
          }
        },
        "model_hints": {
          "authority_invariant": "Person != PrincipalRecord != ActorBinding"
        }
      },
      "Event": {
        "ref": "kip://profiles/cognitive-memory@2.0.0/Event",
        "kind": "ConceptType",
        "description": "Compact episodic anchor describing what happened in a bounded situation.",
        "attributes": {
          "open": true,
          "fields": {
            "event_class": {
              "type": "string",
              "required": false,
              "mutable": true
            },
            "summary": {
              "type": "string",
              "required": true,
              "mutable": true
            },
            "started_at": {
              "type": [
                "timestamp",
                "null"
              ],
              "required": false,
              "mutable": true
            },
            "ended_at": {
              "type": [
                "timestamp",
                "null"
              ],
              "required": false,
              "mutable": true
            },
            "outcome_status": {
              "type": "string",
              "required": false,
              "mutable": true,
              "enum": [
                "success",
                "partial",
                "failure",
                "aborted",
                "unknown"
              ]
            },
            "outcome_summary": {
              "type": "string",
              "required": false,
              "mutable": true
            },
            "context_summary": {
              "type": "string",
              "required": false,
              "mutable": true
            }
          }
        },
        "model_hints": {
          "question": "What happened?",
          "evidence_invariant": "Event summary is not independent Evidence."
        }
      },
      "Experience": {
        "ref": "kip://profiles/cognitive-memory@2.0.0/Experience",
        "kind": "ConceptType",
        "description": "Bounded goal-directed trajectory through state, action, observation, feedback, and outcome.",
        "attributes": {
          "open": true,
          "fields": {
            "experience_class": {
              "type": "string",
              "required": false,
              "mutable": true
            },
            "goal": {
              "type": "string",
              "required": true,
              "mutable": true
            },
            "initial_state_summary": {
              "type": "string",
              "required": false,
              "mutable": true
            },
            "outcome_status": {
              "type": "string",
              "required": true,
              "mutable": true,
              "enum": [
                "success",
                "partial",
                "failure",
                "aborted",
                "unknown"
              ]
            },
            "outcome_summary": {
              "type": "string",
              "required": false,
              "mutable": true
            },
            "started_at": {
              "type": [
                "timestamp",
                "null"
              ],
              "required": false,
              "mutable": true
            },
            "ended_at": {
              "type": [
                "timestamp",
                "null"
              ],
              "required": false,
              "mutable": true
            },
            "step_count": {
              "type": "integer",
              "required": false,
              "mutable": true,
              "minimum": 0
            },
            "surprise": {
              "type": "number",
              "required": false,
              "mutable": true,
              "minimum": 0,
              "maximum": 1
            },
            "learning_value": {
              "type": "number",
              "required": false,
              "mutable": true,
              "minimum": 0,
              "maximum": 1
            },
            "consolidation_status": {
              "type": "string",
              "required": false,
              "mutable": true,
              "enum": [
                "pending",
                "semantic",
                "procedural",
                "completed",
                "archived"
              ]
            }
          }
        },
        "model_hints": {
          "question": "What path was traversed while pursuing a goal?",
          "avoid": [
            "Do not store hidden chain-of-thought."
          ],
          "formation_signals": [
            "multi-step goal pursuit",
            "failure/recovery",
            "prediction error",
            "strategy revision",
            "corrective feedback",
            "novel procedure",
            "counterexample"
          ]
        }
      },
      "ExperienceStep": {
        "ref": "kip://profiles/cognitive-memory@2.0.0/ExperienceStep",
        "kind": "ConceptType",
        "description": "One ordered externally representable step of an Experience trajectory.",
        "attributes": {
          "open": true,
          "fields": {
            "step_kind": {
              "type": "string",
              "required": true,
              "mutable": true,
              "enum": [
                "context",
                "observation",
                "decision",
                "action",
                "feedback",
                "belief_update"
              ]
            },
            "summary": {
              "type": "string",
              "required": true,
              "mutable": true
            },
            "timestamp": {
              "type": [
                "timestamp",
                "null"
              ],
              "required": false,
              "mutable": true
            },
            "tool": {
              "type": [
                "string",
                "null"
              ],
              "required": false,
              "mutable": true
            },
            "result_status": {
              "type": [
                "string",
                "null"
              ],
              "required": false,
              "mutable": true,
              "enum": [
                "success",
                "partial",
                "failure",
                "aborted",
                "unknown",
                null
              ]
            },
            "expected_observation": {
              "type": [
                "string",
                "null"
              ],
              "required": false,
              "mutable": true
            },
            "actual_observation": {
              "type": [
                "string",
                "null"
              ],
              "required": false,
              "mutable": true
            },
            "decision_summary": {
              "type": [
                "string",
                "null"
              ],
              "required": false,
              "mutable": true
            }
          }
        },
        "model_hints": {
          "privacy_invariant": "decision_summary may store concise reusable rationale but never requires hidden chain-of-thought.",
          "ordering_invariant": "Step order lives on the ordered has_step Structural References (virtual edge index); causality requires an explicit caused_by Proposition + Assertion."
        }
      },
      "Insight": {
        "ref": "kip://profiles/cognitive-memory@2.0.0/Insight",
        "kind": "ConceptType",
        "description": "Declarative lesson derived from Evidence or Experience. An optional task_family associates relevant stream material, but an Insight has no Skill trial or lifecycle standing.",
        "attributes": {
          "open": true,
          "fields": {
            "summary": {
              "type": "string",
              "required": true,
              "mutable": true
            },
            "task_family": {
              "type": [
                "string",
                "null"
              ],
              "required": false,
              "mutable": true
            },
            "scope": {
              "type": "string",
              "required": false,
              "mutable": true
            },
            "created_at": {
              "type": [
                "timestamp",
                "null"
              ],
              "required": false,
              "mutable": true
            }
          }
        },
        "model_hints": {
          "epistemic_invariant": "If truth-apt, also represent the reusable claim through Proposition + Assertion."
        }
      },
      "Commitment": {
        "ref": "kip://profiles/cognitive-memory@2.0.0/Commitment",
        "kind": "ConceptType",
        "description": "Prospective memory artifact for an obligation, promise, reminder, follow-up, or intended future action.",
        "attributes": {
          "open": true,
          "fields": {
            "summary": {
              "type": "string",
              "required": true,
              "mutable": true
            },
            "status": {
              "type": "string",
              "required": true,
              "mutable": true,
              "enum": [
                "pending",
                "fulfilled",
                "cancelled",
                "expired",
                "blocked"
              ]
            },
            "created_at": {
              "type": [
                "timestamp",
                "null"
              ],
              "required": false,
              "mutable": true
            },
            "due_at": {
              "type": [
                "timestamp",
                "null"
              ],
              "required": false,
              "mutable": true
            },
            "completed_at": {
              "type": [
                "timestamp",
                "null"
              ],
              "required": false,
              "mutable": true
            },
            "priority": {
              "type": [
                "number",
                "null"
              ],
              "required": false,
              "mutable": true
            }
          }
        },
        "model_hints": {
          "temporal_invariant": "due_at != retention.expires_at",
          "authority_invariant": "Commitment does not automatically schedule or authorize an external action."
        }
      },
      "Skill": {
        "ref": "kip://profiles/cognitive-memory@2.0.0/Skill",
        "kind": "ConceptType",
        "description": "Stable Skill identity, display state and current lifecycle cache. Behavior lives only on current_revision; selecting a new revision resets current standing to proposed atomically.",
        "attributes": {
          "open": false,
          "fields": {
            "skill_class": {
              "type": "string",
              "required": true,
              "mutable": true,
              "enum": [
                "heuristic",
                "workflow",
                "checklist",
                "tool_policy",
                "communication_strategy",
                "diagnostic",
                "recovery",
                "prompt_pattern",
                "code_pattern",
                "subagent_pattern"
              ]
            },
            "summary": {
              "type": "string",
              "required": true,
              "mutable": true
            },
            "failure_modes": {
              "type": [
                "array",
                "object"
              ],
              "required": false,
              "mutable": true
            },
            "counterexamples": {
              "type": [
                "array",
                "object"
              ],
              "required": false,
              "mutable": true
            },
            "status": {
              "type": "string",
              "required": true,
              "mutable": true,
              "enum": [
                "proposed",
                "trialed",
                "adopted",
                "revoked"
              ]
            },
            "created_at": {
              "type": [
                "timestamp",
                "null"
              ],
              "required": false,
              "mutable": true
            }
          }
        },
        "model_hints": {
          "authority_invariant": "Adopted Skill != executable authority.",
          "lifecycle_invariant": "Lifecycle changes and grading refreshes require validated immutable EvaluationRecord. Only trialed -> adopted promotes through a comparative TrialRecord; same-state monitoring follows authorized policy and retains prior adoption evidence; policy withdrawal may have zero outcomes. The acting model never promotes, and revocation is never harder than adoption.",
          "attribution_invariant": "An outcome counts toward this Skill only through an outcome_observation Activity that names an action_gate decision which applied it; sharing the task_family attributes nothing."
        }
      },
      "SleepTask": {
        "ref": "kip://profiles/cognitive-memory@2.0.0/SleepTask",
        "kind": "ConceptType",
        "description": "Durable maintenance work item for a Brain maintenance process.",
        "attributes": {
          "open": true,
          "fields": {
            "task_class": {
              "type": "string",
              "required": true,
              "mutable": true,
              "enum": [
                "consolidate",
                "review_conflict",
                "review_skill",
                "resolve_identity",
                "review_retention",
                "review_derived",
                "refresh_self_model",
                "inspect_quarantine",
                "review_schema"
              ]
            },
            "summary": {
              "type": "string",
              "required": true,
              "mutable": true
            },
            "status": {
              "type": "string",
              "required": true,
              "mutable": true,
              "enum": [
                "pending",
                "running",
                "completed",
                "cancelled",
                "blocked",
                "failed"
              ]
            },
            "priority": {
              "type": [
                "number",
                "null"
              ],
              "required": false,
              "mutable": true
            },
            "created_at": {
              "type": [
                "timestamp",
                "null"
              ],
              "required": false,
              "mutable": true
            },
            "not_before": {
              "type": [
                "timestamp",
                "null"
              ],
              "required": false,
              "mutable": true
            },
            "review_after": {
              "type": [
                "timestamp",
                "null"
              ],
              "required": false,
              "mutable": true
            }
          }
        },
        "model_hints": {
          "authority_invariant": "Semantic assignment, including to $system, does not grant Principal permission."
        }
      },
      "Watch": {
        "ref": "kip://profiles/cognitive-memory@2.0.0/Watch",
        "kind": "ConceptType",
        "description": "Durable attention state: a declared condition under which a committed change (delta) or the absence of one before due_at (silence) deserves the Brain's attention. Firing grants nothing.",
        "attributes": {
          "open": true,
          "fields": {
            "watch_class": {
              "type": "string",
              "required": true,
              "mutable": true,
              "enum": [
                "delta",
                "silence"
              ]
            },
            "summary": {
              "type": "string",
              "required": true,
              "mutable": true
            },
            "condition": {
              "type": [
                "string",
                "object"
              ],
              "required": true,
              "mutable": true,
              "description": "Structured filter over Change Envelope entries — element | slot {subject, predicate} | type, plus optional ops[] and touched[] — with text as the Brain-interpreted fallback; a text-only condition is not runtime-evaluable."
            },
            "due_at": {
              "type": [
                "timestamp",
                "null"
              ],
              "required": false,
              "mutable": true
            },
            "status": {
              "type": "string",
              "required": true,
              "mutable": true,
              "enum": [
                "armed",
                "fired",
                "expired",
                "disarmed"
              ]
            },
            "priority": {
              "type": [
                "number",
                "null"
              ],
              "required": false,
              "mutable": true
            },
            "created_at": {
              "type": [
                "timestamp",
                "null"
              ],
              "required": false,
              "mutable": true
            },
            "fired_at": {
              "type": [
                "timestamp",
                "null"
              ],
              "required": false,
              "mutable": true
            }
          }
        },
        "model_hints": {
          "authority_invariant": "A fired Watch creates attention (SleepTask/wake signal), never an external action or permission.",
          "firing_invariant": "Atomic generation-guarded firing: watch_fire:<id>:<arm_generation>:<seq> or watch_fire:<id>:<arm_generation>:silence:<due_at>. Requires complete authorized coverage; no automatic action authority.",
          "condition_form": "Baseline object members: element, slot {subject, predicate}, type (at least one); ops (create|update|lifecycle|retention|merge|purge|payload_purge); touched (paths); text (fallback)."
        }
      },
      "SelfModel": {
        "ref": "kip://profiles/cognitive-memory@2.0.0/SelfModel",
        "kind": "ConceptType",
        "description": "Derived cognition describing the Brain/agent's model of its own identity, capabilities, limitations, values, preferences, habits, relationships, and goals.",
        "attributes": {
          "open": true,
          "fields": {
            "summary": {
              "type": "string",
              "required": true,
              "mutable": true
            },
            "identity_summary": {
              "type": [
                "string",
                "object",
                "null"
              ],
              "required": false,
              "mutable": true
            },
            "capabilities": {
              "type": [
                "array",
                "object",
                "null"
              ],
              "required": false,
              "mutable": true
            },
            "limitations": {
              "type": [
                "array",
                "object",
                "null"
              ],
              "required": false,
              "mutable": true
            },
            "values": {
              "type": [
                "array",
                "object",
                "null"
              ],
              "required": false,
              "mutable": true
            },
            "preferences": {
              "type": [
                "array",
                "object",
                "null"
              ],
              "required": false,
              "mutable": true
            },
            "goals": {
              "type": [
                "array",
                "object",
                "null"
              ],
              "required": false,
              "mutable": true
            },
            "created_at": {
              "type": [
                "timestamp",
                "null"
              ],
              "required": false,
              "mutable": true
            }
          }
        },
        "model_hints": {
          "authority_invariant": "SelfModel is descriptive cognition, never Governance or Principal identity."
        }
      },
      "WorkingState": {
        "ref": "kip://profiles/cognitive-memory@2.0.0/WorkingState",
        "kind": "ConceptType",
        "description": "Derived, versioned digest of what matters now — the consolidated state a waking Agent resumes from. A derived recall surface served with its basis_seq; never Evidence.",
        "attributes": {
          "open": true,
          "fields": {
            "summary": {
              "type": "string",
              "required": true,
              "mutable": true
            },
            "horizon": {
              "type": [
                "string",
                "null"
              ],
              "required": false,
              "mutable": true
            },
            "basis_seq": {
              "type": "integer",
              "required": true,
              "mutable": true,
              "minimum": 0
            },
            "refreshed_at": {
              "type": [
                "timestamp",
                "null"
              ],
              "required": false,
              "mutable": true
            }
          }
        },
        "model_hints": {
          "evidence_invariant": "WorkingState is never cited as Evidence and never corroborates its own inputs.",
          "identity_hint": "WorkingState answers 'what is my situation'; SelfModel answers 'who am I'. Keep at most one active per actor scope under a stable key."
        }
      },
      "SkillRevision": {
        "ref": "kip://profiles/cognitive-memory@2.0.0/SkillRevision",
        "kind": "ConceptType",
        "description": "Immutable behavior of a stable Skill. All grades, trials, decisions and execution authority bind this revision and its behavior_digest.",
        "attributes": {
          "open": false,
          "fields": {
            "task_family": {
              "type": "string",
              "required": true,
              "mutable": false
            },
            "applicability": {
              "type": [
                "string",
                "array",
                "object"
              ],
              "required": false,
              "mutable": false
            },
            "preconditions": {
              "type": [
                "string",
                "array",
                "object"
              ],
              "required": false,
              "mutable": false
            },
            "procedure": {
              "type": [
                "string",
                "array",
                "object"
              ],
              "required": true,
              "mutable": false
            },
            "success_criteria": {
              "type": [
                "string",
                "array",
                "object"
              ],
              "required": false,
              "mutable": false
            },
            "recovery": {
              "type": [
                "string",
                "array",
                "object"
              ],
              "required": false,
              "mutable": false
            },
            "behavior_digest": {
              "type": "string",
              "required": true,
              "mutable": false,
              "value_schema": {
                "type": "string",
                "minLength": 1,
                "pattern": "^sha256:[0-9a-f]{64}$"
              }
            }
          },
          "value_schema": {
            "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/SkillRevision"
          }
        }
      }
    },
    "predicates": {
      "prefers": {
        "ref": "kip://profiles/cognitive-memory@2.0.0/prefers",
        "kind": "PredicateType",
        "description": "Subject prefers the object over the other options of the same kind: the object's Concept Type lineage partitions the slot, so a newer preference among options of one kind succeeds the older one (Spec §20.15, §25.4) while preferences over different kinds coexist. Each option is therefore a Concept typed by its kind (a ColorScheme, an Editor) from a domain package or the draft vocabulary, never a catch-all type. Truth-sensitive: use Proposition + Assertion + Evidence, not a bare attribute; a preference-pattern summary is an Insight, there is no Preference type.",
        "subject": {
          "concept_types": [
            "kip://profiles/cognitive-memory@2.0.0/Person"
          ]
        },
        "object": {
          "kinds": [
            "Concept"
          ]
        },
        "functional": false,
        "open_world": true,
        "complete": false,
        "functional_by": "object_type"
      },
      "caused_by": {
        "ref": "kip://profiles/cognitive-memory@2.0.0/caused_by",
        "kind": "PredicateType",
        "description": "Effect-to-cause claim between ExperienceSteps: the subject step's outcome/observation was causally produced by the object step. Direction is effect -> cause. This is a truth-sensitive semantic claim (Proposition + Assertion, typically mode observed/inferred with Evidence), never structural topology; step order alone must not be promoted into caused_by.",
        "subject": {
          "concept_types": [
            "kip://profiles/cognitive-memory@2.0.0/ExperienceStep"
          ]
        },
        "object": {
          "concept_types": [
            "kip://profiles/cognitive-memory@2.0.0/ExperienceStep"
          ]
        },
        "functional": false,
        "open_world": true,
        "complete": false
      },
      "same_as": {
        "ref": "kip://profiles/cognitive-memory@2.0.0/same_as",
        "kind": "PredicateType",
        "description": "Unverified identity claim: subject and object are believed to denote the same real-world entity. Feeds identity review; it never auto-merges Concepts and never establishes canonical_id by itself.",
        "subject": {
          "kinds": [
            "Concept"
          ]
        },
        "object": {
          "kinds": [
            "Concept"
          ]
        },
        "functional": false,
        "open_world": true,
        "complete": false
      }
    },
    "facets": {
      "MnemonicState": {
        "ref": "kip://profiles/cognitive-memory@2.0.0/MnemonicState",
        "kind": "FacetDefinition",
        "description": "Mutable mnemonic accessibility/importance state, separate from epistemic confidence and trust.",
        "closed": true,
        "applicable_to": {
          "kinds": [
            "Concept"
          ]
        },
        "fields": {
          "memory_strength": {
            "type": "number",
            "required": false,
            "mutable": true,
            "minimum": 0,
            "maximum": 1
          },
          "salience": {
            "type": "number",
            "required": false,
            "mutable": true,
            "minimum": 0,
            "maximum": 1
          },
          "utility": {
            "type": "number",
            "required": false,
            "mutable": true,
            "minimum": 0,
            "maximum": 1
          },
          "last_metabolized_at": {
            "type": [
              "timestamp",
              "null"
            ],
            "required": false,
            "mutable": true
          },
          "strength_policy": {
            "type": "object",
            "required": false,
            "mutable": true,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:common#/$defs/ArtifactPin"
            }
          },
          "effective_strength": {
            "type": [
              "number",
              "null"
            ],
            "required": false,
            "mutable": false,
            "computed": true,
            "minimum": 0,
            "maximum": 1,
            "description": "Computed when a read is evaluated from memory_strength (base), last_metabolized_at (anchor) and strength_policy; null when any is missing. Never written back."
          }
        },
        "model_hints": {
          "invariants": [
            "memory_strength != Assertion confidence",
            "salience != source trust",
            "utility is the admission bet, revised by outcomes; not truth, salience, or permission",
            "Facet state cannot override Core/Governance fields",
            "Skills carry it too: a Skill's expected usefulness is MnemonicState.utility; its graded record is the computed GradingState view",
            "memory_strength is the last written base; decay is computed (effective_strength), never swept"
          ]
        }
      },
      "GradingState": {
        "ref": "kip://profiles/cognitive-memory@2.0.0/GradingState",
        "kind": "FacetDefinition",
        "description": "Computed, read-only view of the EvaluationRecord that the Skill's current_evaluation references, for the Skill's current_revision: counts of independent attempts, never raw observations. Absent until a validated evaluation exists; a write fails ConstraintViolation.",
        "closed": true,
        "applicable_to": {
          "concept_types": [
            "kip://profiles/cognitive-memory@2.0.0/Skill"
          ]
        },
        "fields": {
          "success_count": {
            "type": "integer",
            "required": false,
            "mutable": false,
            "minimum": 0
          },
          "failure_count": {
            "type": "integer",
            "required": false,
            "mutable": false,
            "minimum": 0
          },
          "graded_count": {
            "type": "integer",
            "required": false,
            "mutable": false,
            "minimum": 0
          },
          "last_verdict_at": {
            "type": [
              "timestamp",
              "null"
            ],
            "required": false,
            "mutable": false
          },
          "revision_ref": {
            "type": "string",
            "required": false,
            "mutable": false
          },
          "evaluation_ref": {
            "type": "string",
            "required": false,
            "mutable": false
          }
        },
        "model_hints": {
          "authority_invariant": "A view of immutable evaluation records; grants no permission and cannot be written."
        },
        "reference_paths": [
          {
            "path": "/revision_ref",
            "target": "element",
            "namespace": "source"
          },
          {
            "path": "/evaluation_ref",
            "target": "element",
            "namespace": "source"
          }
        ],
        "computed": true
      },
      "OutcomeRecord": {
        "ref": "kip://profiles/cognitive-memory@2.0.0/OutcomeRecord",
        "kind": "FacetDefinition",
        "description": "OutcomeRecord: normative contract in brain/KIP-2.0-Validated-Learning.md §3; value_schema is binding.",
        "closed": true,
        "applicable_to": {
          "kinds": [
            "Evidence"
          ]
        },
        "fields": {
          "task_family": {
            "type": "string",
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/OutcomeRecord/properties/task_family"
            }
          },
          "attempt_ref": {
            "type": [
              "string",
              "null"
            ],
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/OutcomeRecord/properties/attempt_ref"
            }
          },
          "metric": {
            "type": "string",
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/OutcomeRecord/properties/metric"
            }
          },
          "window": {
            "type": "string",
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/OutcomeRecord/properties/window"
            }
          },
          "terminal": {
            "type": "boolean",
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/OutcomeRecord/properties/terminal"
            }
          },
          "observation_key": {
            "type": "string",
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/OutcomeRecord/properties/observation_key"
            }
          },
          "observer_config_digest": {
            "type": "string",
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/OutcomeRecord/properties/observer_config_digest"
            }
          },
          "outcome_status": {
            "type": "string",
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/OutcomeRecord/properties/outcome_status"
            }
          },
          "magnitude": {
            "type": [
              "number",
              "null"
            ],
            "required": false,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/OutcomeRecord/properties/magnitude"
            }
          }
        },
        "value_schema": {
          "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/OutcomeRecord"
        },
        "reference_paths": [
          {
            "path": "/attempt_ref",
            "target": "element",
            "namespace": "source"
          }
        ]
      },
      "DecisionRecord": {
        "ref": "kip://profiles/cognitive-memory@2.0.0/DecisionRecord",
        "kind": "FacetDefinition",
        "description": "DecisionRecord: normative contract in profiles/CognitiveMemoryProfile-2.0.md §6.4; value_schema is binding.",
        "closed": true,
        "applicable_to": {
          "kinds": [
            "Activity"
          ]
        },
        "fields": {
          "decision": {
            "type": "string",
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/DecisionRecord/properties/decision"
            }
          },
          "rationale": {
            "type": [
              "string",
              "null"
            ],
            "required": false,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/DecisionRecord/properties/rationale"
            }
          },
          "retrieved_refs": {
            "type": "array",
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/DecisionRecord/properties/retrieved_refs"
            }
          },
          "used_refs": {
            "type": "array",
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/DecisionRecord/properties/used_refs"
            }
          },
          "applied_revisions": {
            "type": "array",
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/DecisionRecord/properties/applied_revisions"
            }
          },
          "basis": {
            "type": "object",
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/DecisionRecord/properties/basis"
            }
          }
        },
        "value_schema": {
          "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/DecisionRecord"
        },
        "attachment": {
          "activity_classes": [
            "action_gate"
          ],
          "terminal_only": true
        },
        "reference_paths": [
          {
            "path": "/retrieved_refs/*",
            "target": "element",
            "namespace": "source"
          },
          {
            "path": "/used_refs/*",
            "target": "element",
            "namespace": "source"
          },
          {
            "path": "/applied_revisions/*",
            "target": "element",
            "namespace": "source"
          }
        ]
      },
      "DependencyBasis": {
        "ref": "kip://profiles/cognitive-memory@2.0.0/DependencyBasis",
        "kind": "FacetDefinition",
        "description": "DependencyBasis: normative contract in SPECIFICATION.md §57.6; value_schema is binding.",
        "closed": true,
        "applicable_to": {
          "kinds": [
            "Activity"
          ]
        },
        "fields": {
          "basis_seq": {
            "type": "integer",
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/DependencyBasis/properties/basis_seq"
            }
          },
          "groups": {
            "type": "array",
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/DependencyBasis/properties/groups"
            }
          },
          "policy_basis": {
            "type": "object",
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/DependencyBasis/properties/policy_basis"
            }
          },
          "queries": {
            "type": "array",
            "required": false,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/DependencyBasis/properties/queries"
            }
          }
        },
        "value_schema": {
          "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/DependencyBasis"
        },
        "reference_paths": [
          {
            "path": "/groups/*/pins/*/id",
            "target": "element",
            "namespace": "source"
          }
        ]
      },
      "AttemptRecord": {
        "ref": "kip://profiles/cognitive-memory@2.0.0/AttemptRecord",
        "kind": "FacetDefinition",
        "description": "AttemptRecord: normative contract in brain/KIP-2.0-Validated-Learning.md §2; value_schema is binding.",
        "closed": true,
        "applicable_to": {
          "kinds": [
            "Activity"
          ]
        },
        "fields": {
          "attempt_id": {
            "type": "string",
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/AttemptRecord/properties/attempt_id"
            }
          },
          "decision_ref": {
            "type": "string",
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/AttemptRecord/properties/decision_ref"
            }
          },
          "applied_revisions": {
            "type": "array",
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/AttemptRecord/properties/applied_revisions"
            }
          },
          "trial_ref": {
            "type": [
              "string",
              "null"
            ],
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/AttemptRecord/properties/trial_ref"
            }
          },
          "context": {
            "type": "object",
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/AttemptRecord/properties/context"
            }
          },
          "environment_digest": {
            "type": "string",
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/AttemptRecord/properties/environment_digest"
            }
          },
          "tool_versions": {
            "type": "object",
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/AttemptRecord/properties/tool_versions"
            }
          },
          "selection_policy": {
            "type": "object",
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/AttemptRecord/properties/selection_policy"
            }
          },
          "preconditions_satisfied": {
            "type": "string",
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/AttemptRecord/properties/preconditions_satisfied"
            }
          },
          "started_at": {
            "type": "string",
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/AttemptRecord/properties/started_at"
            }
          },
          "assignment": {
            "type": "object",
            "required": false,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/AttemptRecord/properties/assignment"
            }
          }
        },
        "value_schema": {
          "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/AttemptRecord"
        },
        "attachment": {
          "activity_classes": [
            "action_attempt"
          ],
          "terminal_only": true
        },
        "reference_paths": [
          {
            "path": "/decision_ref",
            "target": "element",
            "namespace": "source"
          },
          {
            "path": "/applied_revisions/*",
            "target": "element",
            "namespace": "source"
          },
          {
            "path": "/trial_ref",
            "target": "element",
            "namespace": "source"
          }
        ]
      },
      "TrialRecord": {
        "ref": "kip://profiles/cognitive-memory@2.0.0/TrialRecord",
        "kind": "FacetDefinition",
        "description": "TrialRecord: normative contract in brain/KIP-2.0-Validated-Learning.md §4; value_schema is binding.",
        "closed": true,
        "applicable_to": {
          "kinds": [
            "Activity"
          ]
        },
        "fields": {
          "revision_refs": {
            "type": "array",
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/TrialRecord/properties/revision_refs"
            }
          },
          "basis": {
            "type": "object",
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/TrialRecord/properties/basis"
            }
          },
          "rule": {
            "type": "object",
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/TrialRecord/properties/rule"
            }
          },
          "parameters": {
            "type": "object",
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/TrialRecord/properties/parameters"
            }
          },
          "baseline_attempt_refs": {
            "type": "array",
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/TrialRecord/properties/baseline_attempt_refs"
            }
          },
          "baseline_outcome_refs": {
            "type": "array",
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/TrialRecord/properties/baseline_outcome_refs"
            }
          },
          "comparability": {
            "type": "object",
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/TrialRecord/properties/comparability"
            }
          },
          "quota": {
            "type": "integer",
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/TrialRecord/properties/quota"
            }
          },
          "observation_window": {
            "type": "string",
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/TrialRecord/properties/observation_window"
            }
          },
          "replay_artifact": {
            "type": "object",
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/TrialRecord/properties/replay_artifact"
            }
          },
          "evaluation_policy": {
            "type": "object",
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/TrialRecord/properties/evaluation_policy"
            }
          },
          "baseline_mode": {
            "type": "string",
            "required": false,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/TrialRecord/properties/baseline_mode"
            }
          },
          "enrollment": {
            "type": "object",
            "required": false,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/TrialRecord/properties/enrollment"
            }
          }
        },
        "value_schema": {
          "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/TrialRecord"
        },
        "attachment": {
          "activity_classes": [
            "trial_open"
          ],
          "terminal_only": true
        },
        "reference_paths": [
          {
            "path": "/revision_refs/*",
            "target": "element",
            "namespace": "source"
          },
          {
            "path": "/baseline_attempt_refs/*",
            "target": "element",
            "namespace": "source"
          },
          {
            "path": "/baseline_outcome_refs/*",
            "target": "element",
            "namespace": "source"
          }
        ]
      },
      "EvaluationRecord": {
        "ref": "kip://profiles/cognitive-memory@2.0.0/EvaluationRecord",
        "kind": "FacetDefinition",
        "description": "EvaluationRecord: normative contract in brain/KIP-2.0-Validated-Learning.md §4; value_schema is binding.",
        "closed": true,
        "applicable_to": {
          "kinds": [
            "Activity"
          ]
        },
        "fields": {
          "trial_ref": {
            "type": [
              "string",
              "null"
            ],
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/EvaluationRecord/properties/trial_ref"
            }
          },
          "revision_refs": {
            "type": "array",
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/EvaluationRecord/properties/revision_refs"
            }
          },
          "from_status": {
            "type": "string",
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/EvaluationRecord/properties/from_status"
            }
          },
          "to_status": {
            "type": "string",
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/EvaluationRecord/properties/to_status"
            }
          },
          "rule_digest": {
            "type": "string",
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/EvaluationRecord/properties/rule_digest"
            }
          },
          "parameters_digest": {
            "type": "string",
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/EvaluationRecord/properties/parameters_digest"
            }
          },
          "cutoff": {
            "type": "string",
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/EvaluationRecord/properties/cutoff"
            }
          },
          "attempt_refs": {
            "type": "array",
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/EvaluationRecord/properties/attempt_refs"
            }
          },
          "outcome_refs": {
            "type": "array",
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/EvaluationRecord/properties/outcome_refs"
            }
          },
          "excluded_samples": {
            "type": "array",
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/EvaluationRecord/properties/excluded_samples"
            }
          },
          "missing_attempt_refs": {
            "type": "array",
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/EvaluationRecord/properties/missing_attempt_refs"
            }
          },
          "comparison": {
            "type": "object",
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/EvaluationRecord/properties/comparison"
            }
          },
          "replay_artifact": {
            "type": "object",
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/EvaluationRecord/properties/replay_artifact"
            }
          },
          "cohort_artifact": {
            "type": "object",
            "required": false,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/EvaluationRecord/properties/cohort_artifact"
            }
          }
        },
        "value_schema": {
          "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/EvaluationRecord"
        },
        "attachment": {
          "activity_classes": [
            "lifecycle_verdict"
          ],
          "terminal_only": true
        },
        "reference_paths": [
          {
            "path": "/trial_ref",
            "target": "element",
            "namespace": "source"
          },
          {
            "path": "/revision_refs/*",
            "target": "element",
            "namespace": "source"
          },
          {
            "path": "/attempt_refs/*",
            "target": "element",
            "namespace": "source"
          },
          {
            "path": "/outcome_refs/*",
            "target": "element",
            "namespace": "source"
          }
        ]
      },
      "WatchState": {
        "ref": "kip://profiles/cognitive-memory@2.0.0/WatchState",
        "kind": "FacetDefinition",
        "description": "WatchState: normative contract in brain/KIP-2.0-Brain-Runtime.md §2; value_schema is binding.",
        "closed": true,
        "applicable_to": {
          "concept_types": [
            "kip://profiles/cognitive-memory@2.0.0/Watch"
          ]
        },
        "fields": {
          "arm_generation": {
            "type": "integer",
            "required": true,
            "mutable": true,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/WatchState/properties/arm_generation"
            }
          },
          "armed_seq": {
            "type": "integer",
            "required": true,
            "mutable": true,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/WatchState/properties/armed_seq"
            }
          },
          "condition_digest": {
            "type": "string",
            "required": true,
            "mutable": true,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/WatchState/properties/condition_digest"
            }
          },
          "authorization_view": {
            "type": "string",
            "required": true,
            "mutable": true,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/WatchState/properties/authorization_view"
            }
          },
          "consumed_seq": {
            "type": "integer",
            "required": true,
            "mutable": true,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/WatchState/properties/consumed_seq"
            }
          },
          "matched": {
            "type": "boolean",
            "required": true,
            "mutable": true,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/WatchState/properties/matched"
            }
          }
        },
        "value_schema": {
          "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/WatchState"
        }
      },
      "LeaseState": {
        "ref": "kip://profiles/cognitive-memory@2.0.0/LeaseState",
        "kind": "FacetDefinition",
        "description": "LeaseState: normative contract in brain/KIP-2.0-Brain-Runtime.md §3; value_schema is binding.",
        "closed": true,
        "applicable_to": {
          "concept_types": [
            "kip://profiles/cognitive-memory@2.0.0/SleepTask"
          ]
        },
        "fields": {
          "owner": {
            "type": "string",
            "required": true,
            "mutable": true,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/LeaseState/properties/owner"
            }
          },
          "fencing_token": {
            "type": "integer",
            "required": true,
            "mutable": true,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/LeaseState/properties/fencing_token"
            }
          },
          "expires_at": {
            "type": "string",
            "required": true,
            "mutable": true,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/LeaseState/properties/expires_at"
            }
          },
          "attempt_count": {
            "type": "integer",
            "required": true,
            "mutable": true,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/LeaseState/properties/attempt_count"
            }
          }
        },
        "value_schema": {
          "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/LeaseState"
        }
      },
      "CompressionRecord": {
        "ref": "kip://profiles/cognitive-memory@2.0.0/CompressionRecord",
        "kind": "FacetDefinition",
        "description": "CompressionRecord: normative contract in profiles/CognitiveMemoryProfile-2.0.md §10.1; value_schema is binding.",
        "closed": true,
        "applicable_to": {
          "kinds": [
            "Activity"
          ]
        },
        "fields": {
          "source_refs": {
            "type": "array",
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/CompressionRecord/properties/source_refs"
            }
          },
          "extractor_version": {
            "type": "string",
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/CompressionRecord/properties/extractor_version"
            }
          },
          "schema_version": {
            "type": "string",
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/CompressionRecord/properties/schema_version"
            }
          },
          "preserved_fields": {
            "type": "array",
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/CompressionRecord/properties/preserved_fields"
            }
          },
          "known_omissions": {
            "type": "array",
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/CompressionRecord/properties/known_omissions"
            }
          },
          "reencoding_eligible": {
            "type": "boolean",
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/CompressionRecord/properties/reencoding_eligible"
            }
          }
        },
        "value_schema": {
          "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/CompressionRecord"
        },
        "attachment": {
          "activity_classes": [
            "extraction",
            "experience_formation",
            "encoding_review"
          ],
          "terminal_only": true
        },
        "reference_paths": [
          {
            "path": "/source_refs/*",
            "target": "element",
            "namespace": "source"
          }
        ]
      },
      "RecallCoverage": {
        "ref": "kip://profiles/cognitive-memory@2.0.0/RecallCoverage",
        "kind": "FacetDefinition",
        "description": "RecallCoverage: normative contract in profiles/CognitiveMemoryProfile-2.0.md §20.2; value_schema is binding.",
        "closed": true,
        "applicable_to": {
          "kinds": [
            "Activity"
          ]
        },
        "fields": {
          "basis": {
            "type": "object",
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/RecallCoverage/properties/basis"
            }
          },
          "channels": {
            "type": "object",
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/RecallCoverage/properties/channels"
            }
          },
          "unverified_preconditions": {
            "type": "array",
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/RecallCoverage/properties/unverified_preconditions"
            }
          },
          "action_eligible": {
            "type": "boolean",
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/RecallCoverage/properties/action_eligible"
            }
          },
          "plans": {
            "type": "object",
            "required": false,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/RecallCoverage/properties/plans"
            }
          }
        },
        "value_schema": {
          "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/RecallCoverage"
        },
        "attachment": {
          "activity_classes": [
            "recall_coverage"
          ],
          "terminal_only": true
        }
      },
      "MemoryScope": {
        "ref": "kip://profiles/cognitive-memory@2.0.0/MemoryScope",
        "kind": "FacetDefinition",
        "description": "MemoryScope: normative contract in profiles/CognitiveMemoryProfile-2.0.md §20.3; value_schema is binding.",
        "closed": true,
        "applicable_to": {
          "kinds": [
            "Concept",
            "Assertion",
            "Evidence",
            "Activity"
          ]
        },
        "fields": {
          "task_ref": {
            "type": [
              "string",
              "null"
            ],
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/MemoryScope/properties/task_ref"
            }
          },
          "context_refs": {
            "type": "array",
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/MemoryScope/properties/context_refs"
            }
          }
        },
        "value_schema": {
          "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/MemoryScope"
        },
        "reference_paths": [
          {
            "path": "/task_ref",
            "target": "element",
            "namespace": "source"
          },
          {
            "path": "/context_refs/*",
            "target": "element",
            "namespace": "source"
          }
        ]
      },
      "RecordingRepair": {
        "ref": "kip://profiles/cognitive-memory@2.0.0/RecordingRepair",
        "kind": "FacetDefinition",
        "description": "RecordingRepair: normative contract in SPECIFICATION.md §57.8; value_schema is binding.",
        "closed": true,
        "applicable_to": {
          "kinds": [
            "Activity"
          ]
        },
        "fields": {
          "source_ref": {
            "type": "string",
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/RecordingRepair/properties/source_ref"
            }
          },
          "source_digest": {
            "type": "string",
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/RecordingRepair/properties/source_digest"
            }
          },
          "source_locator": {
            "type": "string",
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/RecordingRepair/properties/source_locator"
            }
          },
          "invalidated_refs": {
            "type": "array",
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/RecordingRepair/properties/invalidated_refs"
            }
          },
          "replacement_refs": {
            "type": "array",
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/RecordingRepair/properties/replacement_refs"
            }
          },
          "reason": {
            "type": "string",
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/RecordingRepair/properties/reason"
            }
          },
          "expected_versions": {
            "type": "object",
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/RecordingRepair/properties/expected_versions"
            }
          }
        },
        "value_schema": {
          "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/RecordingRepair"
        },
        "attachment": {
          "activity_classes": [
            "recording_repair"
          ],
          "terminal_only": true
        },
        "reference_paths": [
          {
            "path": "/source_ref",
            "target": "element",
            "namespace": "source"
          },
          {
            "path": "/invalidated_refs/*",
            "target": "element",
            "namespace": "source"
          },
          {
            "path": "/replacement_refs/*",
            "target": "element",
            "namespace": "source"
          }
        ]
      },
      "ProcedureAssessment": {
        "ref": "kip://profiles/cognitive-memory@2.0.0/ProcedureAssessment",
        "kind": "FacetDefinition",
        "description": "ProcedureAssessment: normative contract in brain/KIP-2.0-Validated-Learning.md §5; value_schema is binding.",
        "closed": true,
        "applicable_to": {
          "kinds": [
            "Activity"
          ]
        },
        "fields": {
          "revision_ref": {
            "type": "string",
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/ProcedureAssessment/properties/revision_ref"
            }
          },
          "basis": {
            "type": "object",
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/ProcedureAssessment/properties/basis"
            }
          },
          "criterion": {
            "type": "object",
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/ProcedureAssessment/properties/criterion"
            }
          },
          "evidence_refs": {
            "type": "array",
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/ProcedureAssessment/properties/evidence_refs"
            }
          },
          "result": {
            "type": "string",
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/ProcedureAssessment/properties/result"
            }
          },
          "purpose": {
            "type": "string",
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/ProcedureAssessment/properties/purpose"
            }
          },
          "grants_standing": {
            "type": "boolean",
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/ProcedureAssessment/properties/grants_standing"
            }
          }
        },
        "value_schema": {
          "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/ProcedureAssessment"
        },
        "attachment": {
          "activity_classes": [
            "skill_validation"
          ],
          "terminal_only": true
        },
        "reference_paths": [
          {
            "path": "/revision_ref",
            "target": "element",
            "namespace": "source"
          },
          {
            "path": "/evidence_refs/*",
            "target": "element",
            "namespace": "source"
          }
        ]
      },
      "RestoreReport": {
        "ref": "kip://profiles/cognitive-memory@2.0.0/RestoreReport",
        "kind": "FacetDefinition",
        "description": "RestoreReport: normative contract in KIP-2.0-Capsule-Specification.md §41.7; value_schema is binding.",
        "closed": true,
        "applicable_to": {
          "kinds": [
            "Activity"
          ]
        },
        "fields": {
          "source_capsule": {
            "type": "object",
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/RestoreReport/properties/source_capsule"
            }
          },
          "mode": {
            "type": "string",
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/RestoreReport/properties/mode"
            }
          },
          "source_history_retained": {
            "type": "boolean",
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/RestoreReport/properties/source_history_retained"
            }
          },
          "mapping_artifact": {
            "type": "object",
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/RestoreReport/properties/mapping_artifact"
            }
          },
          "current_validation": {
            "type": "string",
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/RestoreReport/properties/current_validation"
            }
          },
          "authority_transferred": {
            "type": "boolean",
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/RestoreReport/properties/authority_transferred"
            }
          },
          "missing_artifacts": {
            "type": "array",
            "required": true,
            "mutable": false,
            "value_schema": {
              "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/RestoreReport/properties/missing_artifacts"
            }
          }
        },
        "value_schema": {
          "$ref": "urn:kip:2.0:schema:cognitive-records#/$defs/RestoreReport"
        },
        "attachment": {
          "activity_classes": [
            "import"
          ],
          "terminal_only": true
        }
      }
    },
    "structural_fields": {
      "experienced_by": {
        "ref": "kip://profiles/cognitive-memory@2.0.0/experienced_by",
        "kind": "StructuralFieldDefinition",
        "description": "Primary semantic actor who experienced the trajectory.",
        "source": {
          "concept_types": [
            "kip://profiles/cognitive-memory@2.0.0/Experience"
          ]
        },
        "target": {
          "concept_types": [
            "kip://profiles/cognitive-memory@2.0.0/Person"
          ]
        },
        "cardinality": {
          "min": 0,
          "max": 1
        },
        "ordered": false,
        "unique": true
      },
      "has_step": {
        "ref": "kip://profiles/cognitive-memory@2.0.0/has_step",
        "kind": "StructuralFieldDefinition",
        "description": "Ordered Experience trajectory membership.",
        "source": {
          "concept_types": [
            "kip://profiles/cognitive-memory@2.0.0/Experience"
          ]
        },
        "target": {
          "concept_types": [
            "kip://profiles/cognitive-memory@2.0.0/ExperienceStep"
          ]
        },
        "cardinality": {
          "min": 0,
          "max": null
        },
        "ordered": true,
        "unique": true
      },
      "involves": {
        "ref": "kip://profiles/cognitive-memory@2.0.0/involves",
        "kind": "StructuralFieldDefinition",
        "description": "Contextual participation without claiming stronger domain semantics.",
        "source": {
          "concept_types": [
            "kip://profiles/cognitive-memory@2.0.0/Event",
            "kip://profiles/cognitive-memory@2.0.0/Experience"
          ]
        },
        "target": {
          "kinds": [
            "Concept"
          ]
        },
        "cardinality": {
          "min": 0,
          "max": null
        },
        "ordered": false,
        "unique": true
      },
      "mentions": {
        "ref": "kip://profiles/cognitive-memory@2.0.0/mentions",
        "kind": "StructuralFieldDefinition",
        "description": "Retrieval/context mention without a stronger semantic claim.",
        "source": {
          "concept_types": [
            "kip://profiles/cognitive-memory@2.0.0/Event",
            "kip://profiles/cognitive-memory@2.0.0/Experience",
            "kip://profiles/cognitive-memory@2.0.0/Insight"
          ]
        },
        "target": {
          "kinds": [
            "Concept"
          ]
        },
        "cardinality": {
          "min": 0,
          "max": null
        },
        "ordered": false,
        "unique": true
      },
      "derived_from": {
        "ref": "kip://profiles/cognitive-memory@2.0.0/derived_from",
        "kind": "StructuralFieldDefinition",
        "description": "Computed: the inputs of every terminal Activity whose outputs include this artifact. Activity provenance is the authority; this field is a read-only view of it.",
        "source": {
          "concept_types": [
            "kip://profiles/cognitive-memory@2.0.0/Event",
            "kip://profiles/cognitive-memory@2.0.0/Experience",
            "kip://profiles/cognitive-memory@2.0.0/ExperienceStep",
            "kip://profiles/cognitive-memory@2.0.0/Insight",
            "kip://profiles/cognitive-memory@2.0.0/Commitment",
            "kip://profiles/cognitive-memory@2.0.0/Watch",
            "kip://profiles/cognitive-memory@2.0.0/Skill",
            "kip://profiles/cognitive-memory@2.0.0/SleepTask",
            "kip://profiles/cognitive-memory@2.0.0/SelfModel",
            "kip://profiles/cognitive-memory@2.0.0/WorkingState",
            "kip://profiles/cognitive-memory@2.0.0/SkillRevision"
          ]
        },
        "target": {
          "kinds": [
            "Concept",
            "Proposition",
            "Assertion",
            "Evidence",
            "Activity"
          ]
        },
        "cardinality": {
          "min": 0,
          "max": null
        },
        "ordered": false,
        "unique": true,
        "computed": true
      },
      "compiled_from": {
        "ref": "kip://profiles/cognitive-memory@2.0.0/compiled_from",
        "kind": "StructuralFieldDefinition",
        "description": "Computed: the Experience inputs of the skill_compilation or procedural_consolidation Activity that produced this Skill or SkillRevision.",
        "source": {
          "concept_types": [
            "kip://profiles/cognitive-memory@2.0.0/Skill",
            "kip://profiles/cognitive-memory@2.0.0/SkillRevision"
          ]
        },
        "target": {
          "concept_types": [
            "kip://profiles/cognitive-memory@2.0.0/Experience"
          ]
        },
        "cardinality": {
          "min": 0,
          "max": null
        },
        "ordered": false,
        "unique": true,
        "computed": true
      },
      "compiled_by": {
        "ref": "kip://profiles/cognitive-memory@2.0.0/compiled_by",
        "kind": "StructuralFieldDefinition",
        "description": "Computed: the skill_compilation or procedural_consolidation Activity that produced this Skill or SkillRevision.",
        "source": {
          "concept_types": [
            "kip://profiles/cognitive-memory@2.0.0/Skill",
            "kip://profiles/cognitive-memory@2.0.0/SkillRevision"
          ]
        },
        "target": {
          "kinds": [
            "Activity"
          ]
        },
        "cardinality": {
          "min": 0,
          "max": 1
        },
        "ordered": false,
        "unique": true,
        "computed": true
      },
      "consolidated_to": {
        "ref": "kip://profiles/cognitive-memory@2.0.0/consolidated_to",
        "kind": "StructuralFieldDefinition",
        "description": "Computed: the outputs of the consolidation or formation Activities that took this Event or Experience as input.",
        "source": {
          "concept_types": [
            "kip://profiles/cognitive-memory@2.0.0/Event",
            "kip://profiles/cognitive-memory@2.0.0/Experience"
          ]
        },
        "target": {
          "kinds": [
            "Concept"
          ]
        },
        "cardinality": {
          "min": 0,
          "max": null
        },
        "ordered": false,
        "unique": true,
        "computed": true
      },
      "committed_to": {
        "ref": "kip://profiles/cognitive-memory@2.0.0/committed_to",
        "kind": "StructuralFieldDefinition",
        "description": "Semantic maker/owner of a Commitment.",
        "source": {
          "concept_types": [
            "kip://profiles/cognitive-memory@2.0.0/Commitment"
          ]
        },
        "target": {
          "concept_types": [
            "kip://profiles/cognitive-memory@2.0.0/Person"
          ]
        },
        "cardinality": {
          "min": 0,
          "max": 1
        },
        "ordered": false,
        "unique": true
      },
      "owed_to": {
        "ref": "kip://profiles/cognitive-memory@2.0.0/owed_to",
        "kind": "StructuralFieldDefinition",
        "description": "Beneficiary/counterparty of a Commitment.",
        "source": {
          "concept_types": [
            "kip://profiles/cognitive-memory@2.0.0/Commitment"
          ]
        },
        "target": {
          "concept_types": [
            "kip://profiles/cognitive-memory@2.0.0/Person"
          ]
        },
        "cardinality": {
          "min": 0,
          "max": null
        },
        "ordered": false,
        "unique": true
      },
      "assigned_to": {
        "ref": "kip://profiles/cognitive-memory@2.0.0/assigned_to",
        "kind": "StructuralFieldDefinition",
        "description": "Semantic SleepTask/Watch assignment; never Governance permission.",
        "source": {
          "concept_types": [
            "kip://profiles/cognitive-memory@2.0.0/SleepTask",
            "kip://profiles/cognitive-memory@2.0.0/Watch"
          ]
        },
        "target": {
          "kinds": [
            "Concept"
          ]
        },
        "cardinality": {
          "min": 0,
          "max": 1
        },
        "ordered": false,
        "unique": true
      },
      "watches": {
        "ref": "kip://profiles/cognitive-memory@2.0.0/watches",
        "kind": "StructuralFieldDefinition",
        "description": "The cognition a Watch observes. Topology only: watching an element claims nothing about it.",
        "source": {
          "concept_types": [
            "kip://profiles/cognitive-memory@2.0.0/Watch"
          ]
        },
        "target": {
          "kinds": [
            "Concept",
            "Proposition",
            "Assertion",
            "Evidence",
            "Activity"
          ]
        },
        "cardinality": {
          "min": 0,
          "max": null
        },
        "ordered": false,
        "unique": true
      },
      "about": {
        "ref": "kip://profiles/cognitive-memory@2.0.0/about",
        "kind": "StructuralFieldDefinition",
        "description": "Generic topical Profile relation; prefer domain predicates when semantic relation matters epistemically.",
        "source": {
          "concept_types": [
            "kip://profiles/cognitive-memory@2.0.0/Event",
            "kip://profiles/cognitive-memory@2.0.0/Experience",
            "kip://profiles/cognitive-memory@2.0.0/ExperienceStep",
            "kip://profiles/cognitive-memory@2.0.0/Insight",
            "kip://profiles/cognitive-memory@2.0.0/Commitment",
            "kip://profiles/cognitive-memory@2.0.0/Watch",
            "kip://profiles/cognitive-memory@2.0.0/Skill",
            "kip://profiles/cognitive-memory@2.0.0/SleepTask",
            "kip://profiles/cognitive-memory@2.0.0/SelfModel",
            "kip://profiles/cognitive-memory@2.0.0/WorkingState",
            "kip://profiles/cognitive-memory@2.0.0/SkillRevision"
          ]
        },
        "target": {
          "kinds": [
            "Concept"
          ]
        },
        "cardinality": {
          "min": 0,
          "max": null
        },
        "ordered": false,
        "unique": true
      },
      "current_revision": {
        "ref": "kip://profiles/cognitive-memory@2.0.0/current_revision",
        "kind": "StructuralFieldDefinition",
        "description": "Required stable identity / behavior revision link; current selection is guarded and does not transfer standing.",
        "source": {
          "concept_types": [
            "kip://profiles/cognitive-memory@2.0.0/Skill"
          ]
        },
        "target": {
          "concept_types": [
            "kip://profiles/cognitive-memory@2.0.0/SkillRevision"
          ]
        },
        "cardinality": {
          "min": 1,
          "max": 1
        },
        "ordered": false
      },
      "revision_of": {
        "ref": "kip://profiles/cognitive-memory@2.0.0/revision_of",
        "kind": "StructuralFieldDefinition",
        "description": "Required stable identity / behavior revision link; current selection is guarded and does not transfer standing.",
        "source": {
          "concept_types": [
            "kip://profiles/cognitive-memory@2.0.0/SkillRevision"
          ]
        },
        "target": {
          "concept_types": [
            "kip://profiles/cognitive-memory@2.0.0/Skill"
          ]
        },
        "cardinality": {
          "min": 1,
          "max": 1
        },
        "ordered": false
      },
      "current_trial": {
        "ref": "kip://profiles/cognitive-memory@2.0.0/current_trial",
        "kind": "StructuralFieldDefinition",
        "description": "The trial_open Activity carrying the TrialRecord currently grading this Skill's current_revision. Set when a trial opens, cleared when a new revision is selected; never read by a replay, which uses the immutable record.",
        "source": {
          "concept_types": [
            "kip://profiles/cognitive-memory@2.0.0/Skill"
          ]
        },
        "target": {
          "kinds": [
            "Activity"
          ]
        },
        "cardinality": {
          "min": 0,
          "max": 1
        },
        "ordered": false
      },
      "current_evaluation": {
        "ref": "kip://profiles/cognitive-memory@2.0.0/current_evaluation",
        "kind": "StructuralFieldDefinition",
        "description": "The lifecycle_verdict Activity carrying the EvaluationRecord behind this Skill's current standing. Written in the verdict's transaction; the GradingState view is computed from it.",
        "source": {
          "concept_types": [
            "kip://profiles/cognitive-memory@2.0.0/Skill"
          ]
        },
        "target": {
          "kinds": [
            "Activity"
          ]
        },
        "cardinality": {
          "min": 0,
          "max": 1
        },
        "ordered": false
      }
    },
    "enums": {},
    "registry_extensions": {
      "activity_classes": {
        "values": [
          "experience_formation",
          "semantic_consolidation",
          "reflection",
          "procedural_consolidation",
          "skill_compilation",
          "skill_validation",
          "self_model_refresh",
          "mnemonic_metabolism",
          "commitment_review",
          "watch_fire",
          "action_gate",
          "derivation_review",
          "working_state_refresh",
          "outcome_observation",
          "lifecycle_verdict",
          "action_attempt",
          "trial_open",
          "dependency_validation",
          "encoding_review",
          "recall_coverage",
          "identity_repair",
          "erasure_review",
          "recording_repair"
        ],
        "description": "Process records follow Spec §57.6–§57.8, the Validated Learning companion and the Brain Runtime companion. Terminal input/output versions are engine-captured; immutable TrialRecord/EvaluationRecord retain replay inputs."
      }
    }
  },
  "constraints": [
    {
      "id": "cognitive-memory.person-is-not-principal",
      "scope": "kip://profiles/cognitive-memory@2.0.0/Person",
      "kind": "governance_boundary",
      "description": "Person is semantic cognition and cannot authenticate a Principal or create ActorBinding."
    },
    {
      "id": "cognitive-memory.skill-is-not-authority",
      "scope": "kip://profiles/cognitive-memory@2.0.0/Skill",
      "kind": "governance_boundary",
      "description": "Skill class/status/utility/provenance cannot grant executable or tool authority."
    },
    {
      "id": "cognitive-memory.self-model-is-not-governance",
      "scope": "kip://profiles/cognitive-memory@2.0.0/SelfModel",
      "kind": "governance_boundary",
      "description": "SelfModel cannot mutate Principal identity, ActorBinding, Governance, trust, or Schema authority."
    },
    {
      "id": "cognitive-memory.mnemonic-is-not-epistemic",
      "scope": "kip://profiles/cognitive-memory@2.0.0/MnemonicState",
      "kind": "epistemic_boundary",
      "description": "memory_strength/salience cannot rewrite Assertion confidence or source trust."
    },
    {
      "id": "cognitive-memory.structural-is-not-semantic",
      "scope": "StructuralFieldDefinition",
      "kind": "core_invariant",
      "description": "Profile Structural References do not automatically create semantic Propositions."
    },
    {
      "id": "cognitive-memory.failure-first-class",
      "scope": "kip://profiles/cognitive-memory@2.0.0/Experience",
      "kind": "profile_invariant",
      "description": "A failed Experience is valid memory and does not imply low learning value or automatic archival."
    },
    {
      "id": "cognitive-memory.no-hidden-cot",
      "scope": "kip://profiles/cognitive-memory@2.0.0/ExperienceStep",
      "kind": "privacy_boundary",
      "description": "Store observable steps and concise reusable rationale; hidden chain-of-thought is not required."
    },
    {
      "id": "cognitive-memory.commitment-time-separation",
      "scope": "kip://profiles/cognitive-memory@2.0.0/Commitment",
      "kind": "temporal_boundary",
      "description": "Commitment.due_at is distinct from retention expiry and Assertion valid time."
    },
    {
      "id": "cognitive-memory.provenance-conservation",
      "scope": "Profile",
      "kind": "provenance_invariant",
      "description": "Derived Profile cognition preserves lineage and does not create independent Evidence roots merely by transformation."
    },
    {
      "id": "cognitive-memory.import-authority-conservation",
      "scope": "kip://profiles/cognitive-memory@2.0.0/Skill",
      "kind": "capsule_boundary",
      "description": "Imported Skill content is non-authoritative by default and cannot raise destination Governance authority."
    },
    {
      "id": "cognitive-memory.watch-is-not-authority",
      "scope": "kip://profiles/cognitive-memory@2.0.0/Watch",
      "kind": "governance_boundary",
      "description": "A fired Watch creates attention, never an external action, a schedule, or a permission."
    },
    {
      "id": "cognitive-memory.dependency-validity-is-computed",
      "scope": "kip://core@2.0.0/Concept",
      "kind": "epistemic_boundary",
      "description": "Currentness of a derived artifact is the computed _system.dependency_validity (Spec §57.6); no stored reviewer flag replaces it, and needs_review does not retract, reject or hide the artifact."
    },
    {
      "id": "cognitive-memory.working-state-is-not-evidence",
      "scope": "kip://profiles/cognitive-memory@2.0.0/WorkingState",
      "kind": "epistemic_boundary",
      "description": "WorkingState is a derived view served with its basis_seq; it is never cited as Evidence and never corroborates its inputs."
    },
    {
      "id": "cognitive-memory.outcome-origin-separation",
      "scope": "kip://profiles/cognitive-memory@2.0.0/OutcomeRecord",
      "kind": "provenance_invariant",
      "description": "Outcome Evidence is written by instrumentation; an actor's self-report about its own action's result is never Outcome Evidence, and re-typed instrument output is derived_result, not outcome."
    },
    {
      "id": "cognitive-memory.skill-lifecycle-verdict",
      "scope": "kip://profiles/cognitive-memory@2.0.0/Skill",
      "kind": "profile_invariant",
      "description": "Skill lifecycle changes and current_evaluation updates commit with validated immutable EvaluationRecord. Only trialed -> adopted promotes, using at least two independent treatment attempts and an improved comparison against the immutable TrialRecord basis; current_trial only selects the current trial. Same-state monitoring follows authorized policy, retains prior adoption evidence and need not claim new improvement. Trial entry requires task_family; revoked -> trialed opens a new trial before adoption can recur. Policy withdrawal may have zero outcomes; revocation is never harder than adoption. Lifecycle standing, current_trial and current_evaluation do not survive import."
    },
    {
      "id": "cognitive-memory.outcome-attribution",
      "scope": "kip://profiles/cognitive-memory@2.0.0/GradingState",
      "kind": "provenance_invariant",
      "description": "GradingState is a computed view of the EvaluationRecord referenced by current_evaluation; it cannot be written and grants no authority."
    },
    {
      "id": "cognitive-memory.decision-record-is-not-authorization",
      "scope": "kip://profiles/cognitive-memory@2.0.0/DecisionRecord",
      "kind": "authority_boundary",
      "description": "A DecisionRecord records what the gate decided and distinguishes retrieved_refs, used_refs and applied_revisions. Every applied revision also occurs in the action_gate inputs; retrieval alone proves neither use nor causal credit. The record never authorizes an action."
    },
    {
      "id": "cognitive-memory.revision-standing",
      "scope": "Profile",
      "kind": "profile_invariant",
      "description": "Skill behavior is immutable SkillRevision; selection resets caches; all grades and permissions bind exact revision."
    },
    {
      "id": "cognitive-memory.attempt-sampling",
      "scope": "Profile",
      "kind": "profile_invariant",
      "description": "Grade distinct attempt + metric + window aggregates, not observation count; late outcomes retain original trial."
    },
    {
      "id": "cognitive-memory.evaluation-replay",
      "scope": "Profile",
      "kind": "profile_invariant",
      "description": "Immutable TrialRecord and EvaluationRecord retain complete replay inputs under governance and erasure."
    },
    {
      "id": "cognitive-memory.dependency-validity",
      "scope": "Profile",
      "kind": "profile_invariant",
      "description": "Producing/validation Activities pin DependencyBasis; virtual validity is checked before ordinary Recall."
    },
    {
      "id": "cognitive-memory.durable-workers",
      "scope": "Profile",
      "kind": "profile_invariant",
      "description": "When advertised, WatchState generation and LeaseState fencing are enforced at commit and dispatch."
    },
    {
      "id": "cognitive-memory.behavior-fields-only-on-revision",
      "scope": "kip://profiles/cognitive-memory@2.0.0/Skill",
      "kind": "profile_invariant",
      "description": "Behavior fields are forbidden on Skill writes, including UPSERT and open extensions; current_revision is required. Creation binds revision_of/current_revision in the same transaction."
    },
    {
      "id": "cognitive-memory.revision-ownership",
      "scope": "Profile",
      "kind": "profile_invariant",
      "description": "current_revision target must have revision_of pointing back to that stable Skill. Required one-to-one selection and creation closure are validated atomically."
    }
  ],
  "aliases": {
    "concept_types": {},
    "predicates": {},
    "facets": {},
    "structural_fields": {}
  },
  "compatibility": {
    "readable_with": [
      "kip://core@2.0.0"
    ],
    "supersedes": [],
    "breaking_changes": [
      "Skill behavior moved to immutable SkillRevision; old status/tallies are legacy until migrated.",
      "New typed process records and dependency/coverage contracts are required.",
      "Canonical artifacts use kip-jcs-safe-v1 and portable numeric values."
    ]
  },
  "migrations": [],
  "model_hints": {
    "summary": "Stable Skills select immutable SkillRevisions; decisions enroll independent attempts into immutable trials; outcomes are observations, and replayable evaluations produce current standing. See the Validated Learning companion.",
    "minimal_primer": [
      "Event = compact record of what happened",
      "Experience = goal-directed state/action/observation trajectory",
      "ExperienceStep = ordered observable step; no hidden chain-of-thought",
      "caused_by = explicit effect->cause claim between steps; edge order alone is not causality",
      "Skill = stable identity + current_revision; SkillRevision = immutable behavior + task_family + digest; standing and authority bind the revision",
      "Commitment = prospective memory; not automatic scheduling",
      "Watch = armed attention (delta or silence); firing grants nothing",
      "SelfModel = cognition about self; not Governance",
      "WorkingState = what matters now, with its basis_seq; never Evidence",
      "MnemonicState = accessibility/importance/expected usefulness; not confidence; Skills carry it too",
      "GradingState = computed view of the current evaluation's independent attempt counts; absent grades leave proposed/trialed Skills unproven",
      "DecisionRecord = act|ask|defer|silence on an action_gate Activity whose inputs name what was applied; not authorization",
      "OutcomeRecord = task_family + outcome_status on Outcome Evidence; instruments write it, never the graded actor",
      "task_family finds candidate comparison material; baseline membership and comparability are explicit",
      "lifecycle_verdict = deterministic recorded evaluation; the only path between Skill lifecycle states",
      "prefers = preference among options of one kind; a newer preference of the same kind succeeds the older one"
    ],
    "common_confusions": [
      "Event != Experience",
      "Experience != Skill",
      "Skill != executable authority",
      "memory_strength != Assertion confidence",
      "salience != source trust",
      "utility != truth or permission",
      "Person != Principal",
      "SelfModel != Governance",
      "Structural Reference != Proposition",
      "Remote Experience != local autobiography",
      "has_step order != caused_by",
      "same_as claim != merged identity",
      "fired Watch != authorized action",
      "needs_review != retracted",
      "WorkingState != Evidence",
      "Outcome Evidence != the acting model's self-report",
      "adopted != authorized",
      "task_family != attribution",
      "GradingState != utility",
      "current_trial != adoption",
      "DecisionRecord != authorization",
      "succession (world change) != supersession (correction)"
    ]
  },
  "canonicalization": {
    "profile": "kip-jcs-safe-v1",
    "status": "normative-draft",
    "description": "RFC 8785 JCS, narrowed to KIP portable safe integral values; no nonzero underflow; strict decoded-key/Unicode validation; integrity excluded from its own digest."
  },
  "integrity": {
    "digest_profile": "kip-jcs-safe-v1",
    "content_digest": "sha256:3ea9e4591b403dfc196b611c3e5844be95524987ec3cf80664572e780770d8d9",
    "covers": "all top-level fields except integrity",
    "signatures": []
  }
} as SchemaPackage

/** The package id COGNITIVE_MEMORY declares. */
export const COGNITIVE_MEMORY_ID = "kip://profiles/cognitive-memory"

/** The version COGNITIVE_MEMORY declares. */
export const COGNITIVE_MEMORY_VERSION = "2.0.0"

/**
 * KIP General Domain, version 1.0.0.
 *
 * Vendored verbatim from `rs/anda_cognitive_nexus/profiles/general-domain-1.0.0.json`.
 */
export const GENERAL: SchemaPackage = {
  "format": "KIP-Schema-Package",
  "format_version": "2.0-draft",
  "manifest": {
    "package_id": "kip://domains/general",
    "version": "1.0.0",
    "package_ref": "kip://domains/general@1.0.0",
    "name": "KIP General Domain",
    "description": "A deliberately small general-purpose domain vocabulary for personal and team memory: places, organizations, topics and the everyday relations between people and them. Relations it does not name belong in a domain package or the Space's draft vocabulary (Spec §20.16).",
    "publisher": "urn:kip:publisher:ldclabs",
    "purpose": "standard_domain",
    "stability": "normative_draft",
    "executable": false,
    "source_document": "profiles/CognitiveMemoryProfile-2.0.md"
  },
  "dependencies": [
    {
      "package_id": "kip://core",
      "version": "2.0.0",
      "package_ref": "kip://core@2.0.0",
      "required": true
    },
    {
      "package_id": "kip://profiles/cognitive-memory",
      "version": "2.0.0",
      "package_ref": "kip://profiles/cognitive-memory@2.0.0",
      "required": true,
      "content_digest": "sha256:3ea9e4591b403dfc196b611c3e5844be95524987ec3cf80664572e780770d8d9"
    }
  ],
  "definitions": {
    "concept_types": {
      "Place": {
        "ref": "kip://domains/general@1.0.0/Place",
        "kind": "ConceptType",
        "description": "A named geographic location: a city, a region, a country, a venue.",
        "attributes": {
          "open": true,
          "fields": {}
        },
        "model_hints": {
          "summary": "Use for where people live, travel and meet; nest with located_in."
        }
      },
      "Organization": {
        "ref": "kip://domains/general@1.0.0/Organization",
        "kind": "ConceptType",
        "description": "A company, institution, team or community.",
        "attributes": {
          "open": true,
          "fields": {}
        },
        "model_hints": {
          "summary": "Use for employers, schools, clubs and teams."
        }
      },
      "Topic": {
        "ref": "kip://domains/general@1.0.0/Topic",
        "kind": "ConceptType",
        "description": "A subject of interest or expertise.",
        "attributes": {
          "open": true,
          "fields": {}
        },
        "model_hints": {
          "summary": "Use for interests and skills people talk about; not for preferences among options (use prefers)."
        }
      }
    },
    "predicates": {
      "lives_in": {
        "ref": "kip://domains/general@1.0.0/lives_in",
        "kind": "PredicateType",
        "description": "The subject's primary place of residence. A move is one new Assertion from the move; temporal succession ends the previous residence (Spec §25.4).",
        "subject": {
          "concept_types": [
            "kip://profiles/cognitive-memory@2.0.0/Person"
          ]
        },
        "object": {
          "concept_types": [
            "kip://domains/general@1.0.0/Place"
          ]
        },
        "functional": true,
        "open_world": true,
        "complete": false,
        "temporal_conflict": "overlapping_valid_time"
      },
      "located_in": {
        "ref": "kip://domains/general@1.0.0/located_in",
        "kind": "PredicateType",
        "description": "The smaller place lies within the larger one.",
        "subject": {
          "concept_types": [
            "kip://domains/general@1.0.0/Place"
          ]
        },
        "object": {
          "concept_types": [
            "kip://domains/general@1.0.0/Place"
          ]
        },
        "functional": true,
        "open_world": true,
        "complete": false,
        "temporal_conflict": "overlapping_valid_time"
      },
      "works_for": {
        "ref": "kip://domains/general@1.0.0/works_for",
        "kind": "PredicateType",
        "description": "The subject is employed by or works for the organization. People may hold several roles, so the slot is multi-valued; an ended job is the same actor's reject stance from the end date.",
        "subject": {
          "concept_types": [
            "kip://profiles/cognitive-memory@2.0.0/Person"
          ]
        },
        "object": {
          "concept_types": [
            "kip://domains/general@1.0.0/Organization"
          ]
        },
        "functional": false,
        "open_world": true,
        "complete": false,
        "temporal_conflict": "overlapping_valid_time"
      },
      "member_of": {
        "ref": "kip://domains/general@1.0.0/member_of",
        "kind": "PredicateType",
        "description": "The subject belongs to the organization, team or community.",
        "subject": {
          "concept_types": [
            "kip://profiles/cognitive-memory@2.0.0/Person"
          ]
        },
        "object": {
          "concept_types": [
            "kip://domains/general@1.0.0/Organization"
          ]
        },
        "functional": false,
        "open_world": true,
        "complete": false,
        "temporal_conflict": "overlapping_valid_time"
      },
      "knows": {
        "ref": "kip://domains/general@1.0.0/knows",
        "kind": "PredicateType",
        "description": "The subject is acquainted with the other person. Not symmetric by declaration: each direction is its own claim.",
        "subject": {
          "concept_types": [
            "kip://profiles/cognitive-memory@2.0.0/Person"
          ]
        },
        "object": {
          "concept_types": [
            "kip://profiles/cognitive-memory@2.0.0/Person"
          ]
        },
        "functional": false,
        "open_world": true,
        "complete": false,
        "temporal_conflict": "overlapping_valid_time"
      },
      "timezone": {
        "ref": "kip://domains/general@1.0.0/timezone",
        "kind": "PredicateType",
        "description": "The subject's current timezone, as an IANA name or a UTC offset string.",
        "subject": {
          "concept_types": [
            "kip://profiles/cognitive-memory@2.0.0/Person"
          ]
        },
        "object": {
          "literal_types": [
            "string"
          ]
        },
        "functional": true,
        "open_world": true,
        "complete": false,
        "temporal_conflict": "overlapping_valid_time"
      },
      "speaks": {
        "ref": "kip://domains/general@1.0.0/speaks",
        "kind": "PredicateType",
        "description": "A language the subject speaks, as a BCP 47 language tag string.",
        "subject": {
          "concept_types": [
            "kip://profiles/cognitive-memory@2.0.0/Person"
          ]
        },
        "object": {
          "literal_types": [
            "string"
          ]
        },
        "functional": false,
        "open_world": true,
        "complete": false,
        "temporal_conflict": "overlapping_valid_time"
      },
      "interested_in": {
        "ref": "kip://domains/general@1.0.0/interested_in",
        "kind": "PredicateType",
        "description": "The subject has an interest in the topic.",
        "subject": {
          "concept_types": [
            "kip://profiles/cognitive-memory@2.0.0/Person"
          ]
        },
        "object": {
          "concept_types": [
            "kip://domains/general@1.0.0/Topic"
          ]
        },
        "functional": false,
        "open_world": true,
        "complete": false,
        "temporal_conflict": "overlapping_valid_time"
      }
    },
    "facets": {},
    "structural_fields": {}
  },
  "constraints": [],
  "aliases": {
    "concept_types": {},
    "predicates": {},
    "facets": {},
    "structural_fields": {}
  },
  "compatibility": {
    "readable_with": [
      "kip://core@2.0.0",
      "kip://profiles/cognitive-memory@2.0.0"
    ],
    "supersedes": [],
    "breaking_changes": []
  },
  "migrations": [],
  "model_hints": {
    "summary": "Everyday relations for personal and team memory. Prefer these over inventing near-synonyms; a relation not here goes to the draft vocabulary."
  },
  "canonicalization": {
    "profile": "kip-jcs-safe-v1",
    "status": "normative-draft",
    "description": "RFC 8785 JCS, narrowed to KIP portable safe integral values; no nonzero underflow; strict decoded-key/Unicode validation; integrity excluded from its own digest."
  },
  "integrity": {
    "digest_profile": "kip-jcs-safe-v1",
    "content_digest": "sha256:affede508363c623f019a035641299724eb5de888db76e84dea09be682c894f9"
  }
} as SchemaPackage

/** The package id GENERAL declares. */
export const GENERAL_ID = "kip://domains/general"

/** The version GENERAL declares. */
export const GENERAL_VERSION = "1.0.0"

/** Every bundled artifact, in package-id order. */
export const BUNDLED_PACKAGES: readonly SchemaPackage[] = [
  COGNITIVE_MEMORY,
  GENERAL,
]

        ...
        let (field_key, duplicate_key) = if let Some(key) = cbor_attrs.key {
            ...
            if schema_name == "*" {
                field_type_mappings.push(
                    syn::Error::new_spanned(
                        field_ident,
                        "\"*\" is reserved for wildcard maps and cannot name a fixed struct field",
                    )
                    .to_compile_error(),
                );
                continue;
    ...
    quote! {
        impl #impl_generics #name #ty_generics #where_clause {
            ...
            pub fn field_type() -> #root::FieldType {
                Self::try_field_type().expect("invalid FieldTyped declaration; use try_field_type() for fallible construction")
            }

            #[doc = "Builds this type's declaration, rejecting recursive or invalid type graphs with a SchemaError."]
            pub fn try_field_type() -> ::core::result::Result<#root::FieldType, #root::SchemaError> {
                #root::__private::build::<Self>(|| {
                    ::core::result::Result::Ok(#root::FieldType::Map(
                        ::std::collections::BTreeMap::from([#(#field_type_mappings),*])
                    ))
                })
            }
        }

        impl #impl_generics #root::__private::FallibleFieldTyped for #name #ty_generics #where_clause {
            fn try_field_type() -> ::core::result::Result<#root::FieldType, #root::SchemaError> {
                Self::try_field_type()
            }
        }
    }
